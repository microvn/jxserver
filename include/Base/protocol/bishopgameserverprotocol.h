...
enum B2G_PROTOCOL_ID // Bishop ==> Game server
{
	...
	B2G_PROTOCOLINJXSJ_BEGIN =	200,
	B2G_PAYSYSDISCONNECT,						// 通知GS PaySys和Bishop断连 			
	B2G_PAYSYSRECONNECT,						// 通知GS PaySys和Bishop重连
	B2G_ACCOUNT_VERIFYPASSPOD_RESULT,			// 验证PassPod结果返回
};
...
enum G2B_PROTOCOL_ID // Game server ==> Bishop
{
	...
	G2B_PROTOCOLINJXSJ_BEGIN	= 200,
	G2B_ACCOUNT_VERIFYPASSPOD,				// 验证玩家的密保
};
...
struct KG2BPlayerVerifyPasspod : ProtocolHeader2
{
	char  szAccount[_NAME_LEN];		// 帐号
...
struct KB2GPlayerPasspodVerifyResult : ProtocolHeader2
{
	char	szAccount[_NAME_LEN];		// 帐号
	BYTE	byPasspodVersion;			// 密宝版本
	BYTE	byPasspodMode;				// 密宝认证方式
