...
// LLG_TEMP_20080317_防沉迷帐号登录控制
#define E_LIMITPLAYTIME_FOTBIDLOGIN     22 // 因处于防沉迷限制而禁止登录
#define E_LIMITPLAYTIME_IDINFO_EMPTY	23 // 防沉迷身份证未填写
...
#define E_PASSPOD_MATRIX_FAILED				5007
#define E_PASSPOD_MATRIX_CARDINVALID	5008
#define E_PASSPOD_MATRIX_NOTFOUND	5009

#define ACCOUNT_CURRENT_VERSION                     10

typedef enum
{
    AccountUser = 1,                  // 用户进入游戏调用的协议，一般使用KAccountUser结构，返回用AccountUserReturn
    AccountUserLoginInfo,             // 普通用户登陆使用的协议，一般使用KAccountUserLoginInfo结构，返回用AccountUserReturnEx
    ServerAccountUserLoginInfo,       // 服务端用户的登陆使用的协议，一般是网关，Relay等用户登陆使用KServerAccountUserLoginInfo结构，需要验证IP和网卡的MAC地址，使用AccountUserReturn协议返回，结构使用KAccountUserReturnVerify
    ...
    AccountUserLogout,                // 玩家退出游戏协议，一般使用KAccountUserLogout结构，返回使用AccountUserReturn
    AccountChangeExtPoint,            // 玩家修改附加点，或者使用银票（转为时间），一般使用KAccountUserChangeExtPoint结构，使用AccountUserReturn,但是返回结构使用KAccountUserReturnExt2
    ...
    Gameworld2Paysys = 48,            // 通用协议: 游戏世界到Paysys的请求
    Paysys2Gameworld = 49,            // 通用协议: Paysys到游戏世界的回应包
	...
	AccountUserPasspodVerifyEx = 68,
} ACCOUNTHEADTYPE;

// 防沉迷系统限制标记
enum KE_ACCOUNT_LIMITTIME_FLAG
{
	emKACCOUNT_LIMITTIME_FLAG_NOINFO_NOTLIMIT	= 0,	// 无实名信息，不属于防沉迷管辖范围
	emKACCOUNT_LIMITTIME_FLAG_NOINFO_LIMIT		= 1,	// 无实名信息，属于防沉迷管辖范围
	emKACCOUNT_LIMITTIME_FLAG_INFO_NOTLIMIT		= 2,	// 有实名信息，不属于防沉迷管辖范围
	emKACCOUNT_LIMITTIME_FLAG_INFO_LIMIT		= 3,	// 有实名信息，属于防沉迷管辖范围
	emKACCOUNT_LIMITTIME_FLAG_CLOSE             = 255,  // 防沉迷系统未启用
};
...
struct  KAccountUserPasspodVerifyExt : public KAccountUserPassword
{
	DWORD UserIP;
	DWORD UserPort;
	BYTE  byMachineID[16];      // 相同的MachineID的连接被认为是同一台电脑多开
	...
	BYTE  byReserve[14];
	BYTE  byLogoutIfFailed;
	BYTE  byLogoutDeductFlag;
};
...
struct KAccountUserReturnExtIB2 : public KAccountUserReturn // Paysys ==> Bishop, 附加点操作的结果!
{
	unsigned uFlag;
	int      nPlayerDataIndex;  // 用来将返回协议到达GameServer时，表示取得玩家的数据
	BYTE  byReserve[64];
	DWORD nExtPointArray[MAX_EXT_POINT_COUNT];
...
struct KAccountSetChargeFlagRetExt : public KAccountHead
{
	char  szGatewayName[32];    // 网关名称
	int   nZoneChargeFlag;      // 0:免费,1:收费
	union
