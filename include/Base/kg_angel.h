...
enum KG_ROLE_DATA_ERROR_PROCESS
{
    KG_ROLE_NOT_SAVE,
...
public:
    ...
    // @param uParamBufferSize     : [in] buffer size of pvCreateParam point to.
    //                                    maybe large than this function need. 
    // @param pvCreateParam        : [in] point to create information.
    // @param puRoleDataBufferSize : [in] buffer size of pvRoleData point to.
    //                                    maybe large than this function need.
    //                               [out] size of created role
    // @param pvRoleData           : [out] point to buffer contain role data.
    // @param puNameSize           : [out] return role name's size
    // @param ppvName              : [out] point to role name.
    // @param puAccountSize        : [out] return account's size
    // @param ppvAccount           : [out] point to account.
    // @param puRoleListItemSize   : [in] buffer size of pvRoleListItem
    //                                    maybe large than this function need.
    //                               [out] bytes real store to buffer
    // @param pvRoleListItem       : [out] specify buffer to store result
	...
	// 角色数据删除事件
	virtual void OnRoleDeleted(unsigned uRoleDataSize, void *pvRoleData) = 0;
};
...
public:
    ...
    // @return : role name
    virtual char *GetRoleNameFromRoleData(unsigned uBufferSize,  void *pvRoleData) = 0;
    ...
    // @return : true if successful, false if failed.
    virtual int OnSendRoleList(unsigned uBufferSize, void *pvRoleList) = 0;
	...
	virtual BOOL EncryptRolelist(LPBYTE pbyBuffer, UINT uBufferSize) CONST = 0;
	virtual BOOL DecryptSelPlayer(LPCBYTE pbyBuffer, UINT uBufferSize, LPCSTR pszPassword, LPVOID pDBSelPlayer) CONST = 0;
};
