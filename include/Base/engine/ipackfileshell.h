...
public:
	...
	{
		IPACK_FILE_SHELL_OPTION_SPR_SPLIT_FRAME_BALANCE = 0,			//spr文件大小分帧压缩阀值，大于此值才压缩，小于此值整个文件写入，
...
public:
	...
	virtual bool	AddFolderToPak(int nPakIndex, const char* pPakInternalFolder, const char* pFolder, fnAddFolderToPakCallback pCallback) = 0;
	//往打包文件中添加一个子文件
	virtual bool	AddElemToPak(int nPakIndex, const char* pElemFile) = 0;
	virtual bool	AddElemToPak(int nPakIndex, const char* pPakInternalFolder, const char* pElemFile) = 0;
	//删除打包文件中的一个子文件
