...
public:
	...
	// Open a URL
	void Open(LPCSTR lpszURL, bool bNewWindow = true)
	{
		if (bNewWindow)
			::ShellExecuteA(NULL, NULL, GetBrowser(), lpszURL, NULL, SW_SHOWNORMAL);
		else
			::ShellExecuteA(NULL, NULL, lpszURL, NULL, NULL, SW_SHOWNORMAL);
	}

	// Get the default browser from the registry
	LPCSTR GetBrowser(void)
	{
						    ...
						    &hKey) == ERROR_SUCCESS)
		{
			...
			{
				if (::RegQueryValueExA(hKey, NULL, NULL, NULL, (LPBYTE)szBrowse, &cbData) != ERROR_SUCCESS)
				{
					::RegCloseKey(hKey);
					return "";
				}
