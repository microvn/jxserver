...
class KCodePoints
{
	...
	typedef std::map<unsigned, CODEPOINT> KCPMAP;
...
public:
	...
	~KCodePoints()
	{
		...
		KCPMAP::const_iterator it = m_mapCodePoints.begin();
	...
	void DelPoint(unsigned uAddress)
	{
		if (!m_bInitialized)
			return;
		KCPMAP::iterator it = m_mapCodePoints.find(uAddress);
