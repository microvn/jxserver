...
case "$1" in
 up)
  ...
  docker run -d --name jx3mysql --cap-add=NET_ADMIN -e MYSQL_ROOT_PASSWORD=123456 -e MYSQL_DATABASE=jx3_25 \
    mysql:5.6 --lower-case-table-names=1 --max-allowed-packet=20M >/dev/null
  echo -n "mysql init"; for i in $(seq 1 40); do sleep 2; docker exec jx3mysql mysqladmin -uroot -p123456 ping 2>/dev/null|grep -q alive && break; echo -n .; done; echo
  docker exec jx3mysql mysql -uroot -p123456 -e "GRANT ALL PRIVILEGES ON *.* TO 'root'@'127.0.0.1' IDENTIFIED BY '123456'; GRANT ALL PRIVILEGES ON *.* TO 'root'@'%' IDENTIFIED BY '123456'; FLUSH PRIVILEGES;" 2>/dev/null
