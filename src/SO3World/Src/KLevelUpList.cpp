...
BOOL KLevelUpList::LoadLine(ITabFile* piTabFile, int nLine, KLevelUpData& rLevelData, const KLevelUpData& crDefault)
{
    ...
    // -------------- 招架几率 ------------------------------------------------> 
    nRetCode = piTabFile->GetInteger(nLine, "ParryBaseRate", crDefault.nParryBaseRate, &rLevelData.nParryBaseRate);
    (void)nRetCode; /*[endgame] tolerant*/
    ...
    nRetCode = piTabFile->GetInteger(nLine, "WorldChannelDailyLimit", crDefault.nWorldChannelDailyLimit, &rLevelData.nWorldChannelDailyLimit);
    (void)nRetCode; /*[endgame] tolerant*/

    nRetCode = piTabFile->GetInteger(nLine, "ForceChannelDailyLimit", crDefault.nForceChannelDailyLimit, &rLevelData.nForceChannelDailyLimit);
    (void)nRetCode; /*[endgame] tolerant*/

    nRetCode = piTabFile->GetInteger(nLine, "CampChannelDailyLimit", crDefault.nCampChannelDailyLimit, &rLevelData.nCampChannelDailyLimit);
    (void)nRetCode; /*[endgame] tolerant*/

    // -------------------------------------------------------------
    nRetCode = piTabFile->GetInteger(nLine, "MaxContribution", crDefault.nMaxContribution, &rLevelData.nMaxContribution);
    (void)nRetCode; /*[endgame] tolerant*/
