...
void KNpc::RegisterToViewRegion(KRegion* pFromRegion, KRegion* pToRegion)
{
    ...
    // 1. ����Region�����ڽ�Region���Ϊtrue
    m_pScene->SetViewObjectRegisterFlag(pToRegion, true);
    
    
    // 2. ����Region�����ڽ�Region���Ϊfalse,�¾���Ұ�Ľ����ᱻ����Ϊfalse
    if (pFromRegion)
...
void KNpc::LootCurrencyToEligible(KPlayer* pDropTarget, int nType, int nAmount)
{
    ...
    // (NO tong gate — that is LootContribution-specific; these currencies are not tong-gated.)
    dwTeamID = pDropTarget->GetCurrentTeamID();
    if (dwTeamID == ERROR_ID)
    {
...
void KNpc::InitDialogFlag()
{
    ...
    if (
        (m_pTemplate->dwDialogQuestID)              || // ��������ID�ǿ�
        (m_pTemplate->nTotalRate > 0)               || // �����Ի���Ϊ��
        (m_pTemplate->szShopOptionText[0] != '\0')  || // ���̵�
        (m_pTemplate->bHasBank)                     || // ������
        (m_pTemplate->bHasMailBox)                  || // ������
        (m_pTemplate->dwMasterID)                   || // �Ǽ���ѵ��ʦ
        (m_pTemplate->dwCraftMasterID)                 // �����ѵ��ʦ
    )
    {
        bDialogFlag = true;
        goto Exit1;
