...
struct KRECORD_FILE_HEADER
{
    ...
    char szCompileDate[_NAME_LEN];
    char szCompileTime[_NAME_LEN];
    char szSword3Version[_NAME_LEN];
};
#pragma pack()
    ...
    case ermRecord:
        {
            ...
            strncpy(&FileHeader.szCompileDate[0], __DATE__, sizeof(FileHeader.szCompileDate));
            FileHeader.szCompileDate[sizeof(FileHeader.szCompileDate) - 1] = '\0';

            strncpy(&FileHeader.szCompileTime[0], __TIME__, sizeof(FileHeader.szCompileTime));
            FileHeader.szCompileTime[sizeof(FileHeader.szCompileTime) - 1] = '\0';

            piIniFile = g_OpenIniFile("version.cfg");
	        KGLOG_PROCESS_ERROR(piIniFile);

            nRetCode = piIniFile->GetString("Version", "Sword3.version", "", FileHeader.szSword3Version, sizeof(FileHeader.szSword3Version));
            if (!nRetCode)
            {
                nRetCode = piIniFile->GetString("MAIN", "Sword3.version", "", FileHeader.szSword3Version, sizeof(FileHeader.szSword3Version));
                (void)nRetCode; /*[endgame] tolerant*/
            }

            KG_COM_RELEASE(piIniFile);

            KGLogPrintf(KGLOG_INFO, "The sword3 version is %s \n", FileHeader.szSword3Version);
    ...
    case ermReplay:
        {
            ...
            FileHeader.szCompileDate[sizeof(FileHeader.szCompileDate) - 1] = '\0';
            FileHeader.szCompileTime[sizeof(FileHeader.szCompileTime) - 1] = '\0';
            FileHeader.szSword3Version[sizeof(FileHeader.szSword3Version) - 1] = '\0';

            KGLogPrintf(KGLOG_INFO, "The record game server compiled at %s %s \n", FileHeader.szCompileTime, FileHeader.szCompileDate);
            KGLogPrintf(KGLOG_INFO, "The sword3 version is %s \n", FileHeader.szSword3Version);
