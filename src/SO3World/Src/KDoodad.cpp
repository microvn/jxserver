...
#ifdef _SERVER
    if (crDoodadData.nProgressID != INVALID_SCENE_PROGRESS_ID)
    {
        m_nProgressID     = crDoodadData.nProgressID;
    }
#endif
...
void KDoodad::OnBreakPickPrepare()
{
    BOOL bState = false;
...
void KDoodad::CallBreakScriptFunction(KPlayer* pOpener)
{
    BOOL            bRetCode = false;
    ...
    if (bRetCode && g_pSO3World->m_ScriptCenter.IsFuncExist(m_dwScriptID, SCRIPT_ON_BREAK_OPEN))
    {
        int nTopIndex = 0;
        ...
        g_pSO3World->m_ScriptCenter.CallFunction(m_dwScriptID, SCRIPT_ON_BREAK_OPEN, 0);
...
{	
    ...
    bRetCode = g_InRange(pOpener, this, COMMON_PLAYER_OPERATION_DISTANCE);
...
int KDoodad::GetOpenFrame(KPlayer* pOpener)
{
    ...
    if (m_eKind == dkCraftTarget || (m_eKind == dkCorpse && m_bLooted))
    {
        ...
        nProfessionLevel += pOpener->m_ProfessionList.GetProfessionAdjustLevel(pRecipe->dwProfessionID);
        nProfessionLevel = max(0, nProfessionLevel);
...
BOOL KDoodad::PerOpen(KPlayer* pPlayer)
{
    ...
    if (m_pLootList == NULL)
    {
        ...
        else
        {
            ...
            m_pLootList = GenerateLootList(pPlayer, true);
...
KLootList* KDoodad::GenerateLootList(KPlayer* pOpener, BOOL bIsAbsoluteFree)
{
    BOOL				bRetCode		        = false;
    int					nResult			        = 0;
    int                 nMoney                  = 0;
    ...
    for (DWORD dwDropIndex = 0; dwDropIndex < MAX_DROP_PER_DOODAD; ++dwDropIndex)
    {
        ...
        for (DWORD dwCountIndex = 0; dwCountIndex < dwItemCount; ++dwCountIndex)
        {
            bRetCode = g_pSO3World->m_DropCenter.DoodadTemplateDrop(
                pLootList, pOpener, m_dwTemplateID, dwDropIndex, bIsAbsoluteFree
            );
            KGLOG_PROCESS_ERROR(bRetCode);
        }
    }
    ...
    for (DWORD dwDropIndex = 0; dwDropIndex < MAX_DROP_PER_DOODAD; ++dwDropIndex)
    {
        ...
        for (DWORD dwCountIndex = 0; dwCountIndex < dwItemCount; ++dwCountIndex)
        {
            bRetCode = g_pSO3World->m_DropCenter.DoodadClassDrop(
                pLootList, pOpener, m_pTemplate->m_dwClassID, dwDropIndex, m_pTemplate->m_nLevel, bIsAbsoluteFree
            );
            KGLOG_PROCESS_ERROR(bRetCode);
        }
    }
    ...
    bRetCode = g_pSO3World->m_DropCenter.DoodadDropQuestItem(m_dwTemplateID, pOpener, pLootList, bIsAbsoluteFree);
    KGLOG_PROCESS_ERROR(bRetCode);
...
KLootList* KDoodad::GenerateLootListFromNpc(KPlayer* pDropTarget, KNpc* pNpc)
{
    ...
    bRetCode = pLootList->GenerateLooterList(pDropTarget, this);
    KGLOG_PROCESS_ERROR(bRetCode);

    pLootList->GetLooterList(vecLooterList);

    pScene = pDropTarget->m_pScene;
    assert(pScene);
    ...
    if (pNpc->m_bDropNotQuestItemFlag && bCanDrop)
    {
        ...
        {
            dwParentID = pNpc->m_pTemplate->nDropClassID;
            do 
            {
                dwDropClassID = dwParentID;

                bRetCode = g_pSO3World->m_DropCenter.NpcClassDrop(pLootList, vecLooterList, dwDropClassID, pNpc->m_nLevel);
                KGLOG_PROCESS_ERROR(bRetCode);
        ...
        {
            int nItemCount = pNpc->m_pTemplate->nDropCount[nDropIndex];
            for (int nCountIndex = 0; nCountIndex < nItemCount; nCountIndex++)
            {		
                bRetCode = g_pSO3World->m_DropCenter.NpcTemplateDrop(pLootList, vecLooterList, pNpc->m_dwTemplateID, nDropIndex);
                KGLOG_PROCESS_ERROR(bRetCode);
            }
        }
    }
    ...
    // 任务掉落
    bRetCode = g_pSO3World->m_DropCenter.NpcDropQuestItem(pNpc->m_dwTemplateID, pDropTarget, pLootList, vecLooterList);
    KGLOG_PROCESS_ERROR(bRetCode);
    ...
    for (int nLooterIndex = 0; nLooterIndex < (int)vecLooterList.size(); nLooterIndex++)
    {
        ...
        pPlayer->m_QuestList.OnKillNpc(pNpc->m_dwTemplateID);
        pPlayer->m_PQList.OnKillNpc(pNpc->m_dwTemplateID);
    }

    // 金钱掉落
    if (bCanDrop)
    {
        nMoney = g_pSO3World->m_DropCenter.NpcDropMoney(pNpc->m_dwTemplateID);
