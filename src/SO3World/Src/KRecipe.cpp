...
)
{
	...
	// 专业等级是否足够
	nProfessionLevel = pCaster->m_ProfessionList.GetProfessionLevel(dwProfessionID);
    nProfessionLevel += pCaster->m_ProfessionList.GetProfessionAdjustLevel(dwProfessionID);

	KG_PROCESS_ERROR_RET_CODE(nProfessionLevel >= (int)dwRequireProfessionLevel, crcTooLowProfessionLevel);
    
    if (dwProfessionIDExt != 0)
    {
        int nProfessionLevelExt = 0;
        // 是否学了扩展专业
	    ...
	    // 扩展专业等级是否足够
	    nProfessionLevelExt = pCaster->m_ProfessionList.GetProfessionLevel(dwProfessionIDExt);
        nProfessionLevelExt += pCaster->m_ProfessionList.GetProfessionAdjustLevel(dwProfessionIDExt);
	    KG_PROCESS_ERROR_RET_CODE(nProfessionLevelExt >= (int)dwRequireProfessionLevelExt, crcTooLowExtProfessionLevel);
    }
	...
	if (nEquipmentType >= 0)
	{
		...
		KG_PROCESS_ERROR_RET_CODE(pItem && pItem->m_Common.nDetail == nEquipmentType, crcWeaponError);
	}
...
CRAFT_RESULT_CODE KCraftRecipe::CanCast(KPlayer* pCaster, KTarget& Target)
{
	...
	{ 
		...
		KG_PROCESS_ERROR_RET_CODE(SearchDoodad.m_pResult, crcRequireDoodad);
	}
	...
	for (nIndex = 0; nIndex < MAX_MATERIAL; nIndex++)
	{
		...
		dwCurrentItemCount = pCaster->m_ItemList.GetItemAmountInEquipAndPackage(
            dwRequireItemType[nIndex], dwRequireItemIndex[nIndex]
        );
...
CRAFT_RESULT_CODE KCraftRead::CanCast(KPlayer* pCaster, KTarget& Target)
{
    ...
    if (dwCreateItemTab != 0 && dwCreateItemIndex != 0)
    {
        int nFreeRoom = pCaster->m_ItemList.GetAllPackageFreeRoomSize();
        KG_PROCESS_ERROR_RET_CODE(nFreeRoom >= 1, crcInventoryIsFull);    
    }
...
CRAFT_RESULT_CODE KCraftCopy::CanCast(KPlayer* pCaster, KTarget& Target)
{
    ...
    {
        bRetCode = pCaster->m_BookList.IsBookMemorized(dwID, dwSubID);
        if (!bRetCode) // 如果是没有阅读过的书籍,目标必须是doodad
        {
            KDoodad* pDoodad = NULL;
            TARGET_TYPE eType = Target.GetTargetType();
            KG_PROCESS_ERROR(eType == ttDoodad);

            bRetCode = Target.GetTarget(&pDoodad);
            KG_PROCESS_ERROR(bRetCode);

            KG_PROCESS_ERROR(pDoodad->m_pTemplate && pDoodad->m_pTemplate->m_dwTemplateID == dwDoodadTemplateID);

            bRetCode = g_InRange(pCaster, pDoodad, COMMON_PLAYER_OPERATION_DISTANCE);
            KG_PROCESS_ERROR(bRetCode);
        }
    }
