...
BOOL KCharacter::Init(void)
{
	...
	m_nStunStrikeRate                       = 0;
	m_nKnockedBackRate                      = KILO_NUM; // Ĭ�� 100% ���ˡ�
	m_nKnockedOffRate                       = KILO_NUM; // Ĭ�� 100% ���ɡ�
    m_nKnockedDownRate                      = KILO_NUM; // Ĭ�� 100% ����
    m_nRepulsedRate                         = KILO_NUM; // Ĭ�� 100% ������
    m_nPullRate                             = KILO_NUM; // Ĭ�� 100% ����
...
SKILL_RESULT_CODE KCharacter::CastSkill(DWORD dwSkillID, DWORD dwSkillLevel, KTarget& rTarget)
{
    ...
    KGLOG_PROCESS_ERROR(pSkill->m_pBaseInfo->nCastMode != scmItem); // �Ե���ʹ�õļ��ܲ����ߵ������̡�
...
BOOL KCharacter::Halt(void)
{
    ...
    KG_PROCESS_ERROR(m_pRegion); // ��ұ����Ѿ��ڳ�����
...
BOOL KCharacter::CheckDie()
{
	...
	{        
        ...
        if (dwTargetID != ERROR_ID && (eState == pksDueling || eState == pksDuelOutOfRange))
        {
            if (dwLoserID == m_dwID && (dwTargetID == m_dwKillerID || m_dwID == m_dwKillerID))
            {
                ...
                if (pTarget && pTarget->m_bToDie) // ����Ŀ�걻����checkdieŪ��
                {
    ...
    DoActionIdle(); // �жϳ���һ��ʱ��ļ��ܣ�������
    DoAutoCastSkillIdle(); // �жϳ���һ��ʱ���ͷŵ��Զ����ܣ����Զ�����
...
BOOL KCharacter::GoToHell(DWORD dwKiller)
{
    ...
    {
        KPlayer* pPlayer    = (KPlayer*)this;
        DWORD    dwLoserID  = pPlayer->m_PK.GetLoserID(); //  ����֮ǰ�Ѿ����ù�Loser��(���"GoToHell")
...
void KCharacter::ReviseAttribValue()
{
    ...
    MAKE_IN_RANGE(m_nCurrentLife, 1, m_nMaxLife); // �����Ѫ��������1
...
KSKILL_HIT_RESULT KCharacter::JudgeSkillHit(KSKILL_BULLET* pBullet)
{
    ...
    if (pSkill->m_pBaseInfo->bIsExactHit)   // һ�����У����������ܣ�Miss���м�
    {
        goto Exit1;
    }
    ...
    if (nSkillKindType == sktPhysics && pSkill->m_pBaseInfo->nEffectType == scetHarmful) // ֻ���⹦�ж������м�
    {
...
Exit1:
    ...
    if (pSkill->m_pBaseInfo->nEffectType != scetBeneficial)     // ���漼�ܲ�������������ֵ
    {
...
BOOL KCharacter::ApplyBullet(KSKILL_BULLET *pBullet)
{
    ...
    // -------------------- AI�����¼� -------------------------------------->>
    if (
        pSkill->m_pBaseInfo->nEffectType == scetHarmful &&  // �Ǽ���Ч���ļ��ܲ�֪ͨ
        pSkillSrc->m_dwID != m_dwID                         // �����ͷ��߲����ڣ����߼����ͷ������Լ�����֪ͨ
    )
    {
    ...
    m_SkillDebug.Trace(
        1, "[���ܵ���][%d]:[��������] SkillResult:(%s-%u,%u) %d From(%s-%u) To(%s-%u)",
    ...
    if (bDstIsPlayer)
    {
        ...
        if (pSkill->m_nDismountingRate > 0 && pDstPlayer->m_bOnHorse)
        {
            ...
            m_SkillDebug.Trace(
                2, "[���ܵ���][%d]:[��������] (%s-%u) ������:<%s> By %s-%u From %s-%u, ������:%d, ����ֵ:%d ���ֵ:%d",
    ...
    if (pSkill->m_pBaseInfo->bCauseBeatBack && nDamageValue < 0)
    {
        ...
        {
            m_SkillDebug.Trace(
                2, "[���ܵ���][%d]:[��������] Skill:(%s-%u,%u) ����:<�ɹ�>���ѱ�����<%d>��, Frame��%d/%d From(%s-%u) To(%s-%u)",
...
BOOL KCharacter::ConcludeResult(DWORD dwBulletID, BOOL bCriticalStrikeFlag, int* pnRetDamageValue)
{
    ...
    // �˺�����,����֮����Ѿ�����Damage��ʱ��۳���,����������DeltaLife��ʱ��Ҫ���ظ�����!
    nDst_DeltaLife = 0
    ...
    *pnRetDamageValue = nDst_DeltaLife;
    ...
    if (m_nCurrentLife > m_nMaxLife && pSkillSrc) // ��������
    {
	...
	{
        // src �� dst ������,ʹ src���������(ʵ�ʿ�������ȡ����֮���), ��ʹ���г��src���˸����src(����˵һ����dst���src).
    ...
    // ���ܻ����Caster �ܵ����˺�(���練���˺�)����������(����͵ȡ����)ǡ�õ���,�������ּ���̫С��,��������
    // ������ֵ�仯��nSrc_DeltaLifeΪ0ʱ,����ͬ��,ע��,�����ͷ������˺�������ֻ���ܵ�"�����˺�"��"͵ȡ����"Ӱ��
    // ������������������"��"֮�ڵ�Ч��
    // ���src��dst��ͬһ���ˣ���ô����Ǵ�ŵ�dst�ϵģ�����Ͳ��ö�һ��ͬ����
...
BOOL KCharacter::ProcessPull(KCharacter* pCharacter, int nVelocityXY)
{
    ...
    KG_PROCESS_SUCCESS(nApplyOdds + m_nPullRate < KILO_NUM); // ����������жϺͻ���һ������ֵ

    nDistanceCharacter = m_nTouchRange + pCharacter->m_nTouchRange + CELL_LENGTH * 2; // �����˼������CELL
...
BOOL KCharacter::ProcessRepulsed(int nFrame)
{
    ...
    KG_PROCESS_SUCCESS(nApplyOdds + m_nRepulsedRate < KILO_NUM); // ����������жϺͻ���һ������ֵ
...
BOOL KCharacter::ProcessTherapy(int nTherapyValue)
{
    ...
    nDeltaLife = nDeltaLife * (KILO_NUM + m_pSkillAttributeParam->nDamageAddPercent) / KILO_NUM;    // �������żӳ�
...
BOOL KCharacter::ProcessDamage(int nDamageType, int nDamageValue)
{
    ...
    if (nSkillSrcLevel == 0)    // �Ͳ߻�Լ����ȡ����Src�ȼ����Ͱ��Լ��ĵȼ��㡣
        nSkillSrcLevel = m_nLevel;

    nSkillSrcLevel = max(nSkillSrcLevel, 1);    // ��֤�����0
    ...
    case sktPhysics:
        ...
        // -------------- physics ---------------------------------------       
        {
            ...
            if (m_pSkillAttributeParam->bParray) // ���м�
            {
            ...
            m_SkillDebug.Trace(
                2, "[���ܵ���][%d]:[�⹦�˺�����] �⹦�˺�:%d, �⹦�˺�����:%d ����ϵ��:%d �˺�����:%d/%d ħ����:%d/%d ����:%d/%d �⹦�˺��ӳ�:%d To(%s-%d)",
    ...
    case sktSolarMagic:
       ...
       
        {
            ...
            m_SkillDebug.Trace(
                2, "[���ܵ���][%d]:[�����ڹ��˺����] �����ڹ��˺�:%d, �����ڹ��˺�����:%d ����ϵ��:%d �˺�����:%d/%d ħ����:%d/%d ����:%d/%d �����ڹ��˺��ӳ�:%d To(%s-%u)",
                g_pSO3World->m_nGameLoop, DamageCalculateParam.nDamageValue,
                DamageCalculateParam.nDstResistPercent, DamageCalculateParam.nDstDefenceCoefficient,
    ...
    case sktPoison:
        ...
        }
        {
            ...
            m_SkillDebug.Trace(
                2, "[���ܵ���][%d]:[���˺����] ���˺�:%d, ���˺�����:%d ����ϵ��:%d �˺�����:%d/%d ħ����:%d/%d ����:%d/%d ���˺��ӳ�:%d To(%s-%u)",
                g_pSO3World->m_nGameLoop, DamageCalculateParam.nDamageValue,
                DamageCalculateParam.nDstResistPercent, DamageCalculateParam.nDstDefenceCoefficient,
    ...
    if (m_pSkillAttributeParam->bInsight) // ʶ����������˺�
    {
    ...
    DamageCalculateParam.nDamageValue = DamageCalculateParam.nDamageValue * (KILO_NUM + m_pSkillAttributeParam->nDamageAddPercent) / KILO_NUM;    // �������żӳ�
    ...
    if (pSkillSrc && IS_PLAYER(pSkillSrc->m_dwID)) // �����������Player�������Ӫ�Թ���������
    {
...
BOOL KCharacter::CalculateDamage(const KDAMAGE_CALCULATE_PARAM &crParam)
{
    ...
    if (pSkillSrc)
    {
        ...
        {
            nDamageToDst = nDamageToDst * (KILO_NUM + m_pSkillAttributeParam->nAddDamagePercentByDstMoveState) / KILO_NUM; // Ŀ���ƶ�״̬���˺��ļӳ�
        }
    }
    ...
    *crParam.pnDstMaxTypedDamageManaShield -= nTypedManaShieldDamage;
    ...
    // �˺�����: �ٷֱȷ������Լ����Լ��ܵ����˺���ʩ�Ӹ�����ʩ���ߣ���������ֻ�ǰ��˺�ʩ�Ӹ�����ʩ���ߣ����������Լ��ܵ����˺�
...
)
{
    ...
    for (it = SkillEvent.begin(); it != itEnd; ++it)
    {
        ...
        {
            m_SkillDebug.Trace(
                2, "[���ܵ���][%d]:[�����¼�] (%s-%u) �� �����¼���%u ����,��ʩ�ż��� (%u,%u) By %s-(%u,%d) From (%s-%u)",
...
void KCharacter::CheckBeatBreak(KSkill* pSkill)
{
    ...
    m_SkillDebug.Trace(
        1, "[���ܵ���][%d]:[��������] (%s-%u) �� (%s-%u,%d) ����ϡ�By %s-(%u,%u) From %s-%u  ��ϸ���:%d ����ϸ���:%d ������ֵ:%d",
        g_pSO3World->m_nGameLoop, m_szName, m_dwID,
...
void KCharacter::CheckBeatBack()
{
    ...
    {
        m_SkillDebug.Trace(
            2, "[���ܵ���][%d]:[��������] Skill:(%s-%u,%u) ����:<�ɹ�>���ѱ�����<%d>��, Frame: %d From(%s-%u) To(%s-%u)",
            g_pSO3World->m_nGameLoop,
            pSkill->m_pBaseInfo->szSkillName, pSkill->m_pBaseInfo->dwSkillID, pSkill->m_dwLevel,
            m_OTActionParam.nBeatBackCount,
            nBeatBackFrame,
...
BOOL KCharacter::OnPickPrepare()
{
    ...
    KG_PROCESS_SUCCESS(g_pSO3World->m_nGameLoop < m_OTActionParam.nEndFrame);
    // ��PickPrepareʱ�䵽����ɹ�����Doodad���е��������Picking
...
BOOL KCharacter::OnPicking()
{
    ...
    if (pDoodad->m_pLootList == NULL)
    {
        ...
        KGLOG_PROCESS_ERROR(IS_PLAYER(m_dwID));
        g_PlayerServer.DoFinishLoot((KPlayer*)this, pDoodad); // doodadû��lootlist��Ӧ�ý���ʰȡ
        goto Exit0;
    }
#endif	
...
void KCharacter::ProcessAutoFly()
{
    ...
    if (m_nMoveFrameCounter >= pTrack->nFrame)
    {
        ...
        if (IS_PLAYER(m_dwID)) // NPCû�п�ͨ��ͨ���б���û�������Ͱ��ɵ��ж�
        {
            KPlayer* pThis          = (KPlayer*)this;
            int      nNextTrackID   = 0;
        ...
        if (pCurrentNode->dwMapID != pToNode->dwMapID)
        {
            ...
            // �����Ѿ�����һ֡�ˡ��涨���еĿ��ͼ��Track����1֡��
            // �ȵġ���ô�л���Ŀ�ĵ�ͼ�󣬻��Զ�������һ��·���㡣
            pThis = (KPlayer*)this;
...
int  KCharacter::GetFrictionDirection()
{
    ...
    // ע��,������Ϊ���ܹ��ܶ��,����ܶ�����ֻ�������������"���ٶ�",����,Ҫ��nMyV����б�·ֽ�,Ȼ�����뵱ǰ��
    // ˲ʱ�ٶȽ��кϳ�,��ȻҪע��,����Ҫ�������Ħ�����ķ���,����,˲ʱ�ٶȵķ���Ҫȡ�䷴���������м���

    nMyVBase = (int)sqrt(
        (float)SIN_COS_NUMBER * SIN_COS_NUMBER + (float)nCosThetaAlpha * nCosThetaAlpha / nCosBeta * nSinBeta / nCosBeta * nSinBeta
    );

    // ע��,"˲ʱ�ٶ�"����ʵ�ٶȳ�����һ��ϵ��"VELOCITY_ZOOM_COEFFICIENT"��,����Ҫ���ٶȺϳ�,����ҲҪ��ϵ��
    // ע���ֹ��ֵ���
...
BOOL KCharacter::ProcessAcceleration()
{
    ...
    nAlpha = m_pCell->GetGradientDirection();
    nBeta  = m_pCell->GetGradientDegree();
    nTheta = GetFrictionDirection(); // ȡĦ������ˮƽ����ͶӰ�ķ���

    // ���㻬��Ħ�����Ĵ�С(����ʵ�����ǻ���Ħ��������ļ��ٶȵĴ�С)
    if (m_pCell->m_BaseInfo.dwCellType != ctWater)
    {
...
int KCharacter::TryMove(int nStepLength, int nDirection, int* pnObstacleDirection)
{
    ...
    if (m_bFightState && IS_NPC(m_dwID) && bIsChase)
    {
        ...
        if (m_pScene->m_bDoNotGoThroughRoof)    // ���ڸ�������NPC����������
...
BOOL KCharacter::ProcessDropDamage()
{
    ...
    KG_PROCESS_ERROR(m_eMoveState != cmsOnDash); // ����ʱ�򲻼���ˤ�ˣ���ֹ��������ʱ��ˤ��
...
inline BOOL IsBattleEnemy(int nSelfSide, int nDestSide, DWORD dwRelationMask)
{
    ...
    if (nSelfSide > nDestSide)  // ����������ǰ�һ��4*4�ľ���ѹ����һ��DWORD�
    {                           // ���ھ���������ǶԳƵģ�����ֻȡX <= Y�Ĳ��֡�
        int nTemp = nSelfSide;
...
// ����������ص��Ǵ�pSrcCharacter��pDstCharacter�Ĺ�ϵ��ĳЩ��ϵ�ǵ����(PK�ͷ�֮����)��
// ���Դ�pSrcCharacter��pDstCharacter�ʹ�pDstCharacter��pSrcCharacter�Ľ�����ܲ�һ��
...
BOOL KCharacter::SetForceID(DWORD dwForceID)
{
    ...
    if (g_pGameWorldUIHandler && g_pGameWorldRepresentHandler)
    {
	    if (m_dwID == g_pSO3World->m_dwClientPlayerID)
	    {
            ...
            g_pGameWorldUIHandler->OnCurrentPlayerForceChanged(param); // ˢ��Player���������
        ...
        else
        {
            ...
            g_pGameWorldRepresentHandler->OnCharacterUpdateTitleMark(this); // ����������ΧNpc��������
