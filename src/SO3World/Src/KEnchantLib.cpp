...
BOOL KEnchantLib::Init()
{
    ...
    for (int nIndex = 2; nIndex <= nHeight; nIndex++)
    {
        ...
        bRetCode = piFile->GetInteger(nIndex, "TabType", 0, (int*)&EnchantInfo.dwTabType);
        (void)bRetCode; /*[endgame] tolerant*/
        
        bRetCode = piFile->GetInteger(nIndex, "TabIndex", 0, (int*)&EnchantInfo.dwTabIndex);
        (void)bRetCode; /*[endgame] tolerant*/

        bRetCode = piFile->GetInteger(nIndex, "PackageSize", 0, &EnchantInfo.nPackageSize);
        (void)bRetCode; /*[endgame] tolerant*/

#ifdef _CLIENT
        bRetCode = piFile->GetInteger(nIndex, "UIID", 0, (int*)&EnchantInfo.dwUIID);
        (void)bRetCode; /*[endgame] tolerant*/
#endif
...
KENCHANT* KEnchantLib::GetEnchantInfo(DWORD dwEnchantID)
{   
    KENCHANT_MAP::iterator it = m_Enchant.find(dwEnchantID);
    if (it != m_Enchant.end())
    {
        return &it->second;
    }
