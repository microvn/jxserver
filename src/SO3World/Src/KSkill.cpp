...
SKILL_RESULT_CODE KSkill::CanCast(KCharacter* pSrcCharacter, KTarget &rTarget)
{
    ...
    case scmTargetLeader:
        if (IS_PLAYER(pSrcCharacter->m_dwID))
        {
            ...
            DWORD           dwTeamID    = pCaster->GetCurrentTeamID();
            ...
            KG_PROCESS_ERROR_RET_CODE(pTeam, srcNotTeam);

            pLeader = g_pSO3World->m_PlayerSet.GetObj(
                pTeam->MemberGroup[nGroupIndex].dwFormationLeader
            );
            KGLOG_PROCESS_ERROR_RET_CODE(pLeader, srcNotTeam);
#else
            pLeader = g_pSO3World->m_PlayerSet.GetObj(
...
SKILL_RESULT_CODE KSkill::Cast(KCharacter* pDisplayCaster, KCharacter* pLogicCaster, KTarget& rTarget, const KSKILL_RECIPE_KEY& crRecipeKey)
{
    ...
    case scmItem:
        assert(false); // 对道具释放技能，绝对不允许进入该流程。
        break;
    ...
    case scmTeamArea:
        nRetCode = CastOnTeamArea(pDisplayCaster, pLogicCaster, crRecipeKey);
        break;
...
SKILL_RESULT_CODE KSkill::CheckCost(KCharacter *pCaster)
{
    ...
    KG_PROCESS_ERROR_RET_CODE(pCaster->m_nCurrentRage     >= nCostRage,     srcNotEnoughRage);
...
void KSkill::DoCost(KCharacter* pCaster)
{
    ...
    pCaster->m_nCurrentRage     -= nCostRage;
...
SKILL_RESULT_CODE KSkill::CheckTargetType(KCharacter *pCaster, KTarget &rTarget)
{
    ...
    KG_PROCESS_ERROR(pTargetCharacter->m_pScene);
    ...
    bRelation =  (nRelation & m_pBaseInfo->nTargetRelationMask);

#ifdef _SERVER
    bRetCode = g_pSO3World->m_MentorCache.IsMentorship(pTargetCharacter->m_dwID, pCaster->m_dwID);
    bMentor = (bRetCode & m_pBaseInfo->bTargetRelationMentor);

    bRetCode = g_pSO3World->m_MentorCache.IsMentorship(pCaster->m_dwID, pTargetCharacter->m_dwID);
    bApprentice = (bRetCode & m_pBaseInfo->bTargetRelationApprentice);
#endif

#ifdef _CLIENT
    bRetCode = g_pSO3World->m_MentorClient.IsMyMentor(pTargetCharacter->m_dwID);
    bMentor = (bRetCode & m_pBaseInfo->bTargetRelationMentor);

    bRetCode = g_pSO3World->m_MentorClient.IsMyApprentice(pTargetCharacter->m_dwID);
    bApprentice = (bRetCode & m_pBaseInfo->bTargetRelationApprentice);
#endif

    KG_PROCESS_ERROR(bRelation || bMentor || bApprentice);
...
SKILL_RESULT_CODE KSkill::CheckFormationRequire(KCharacter *pCaster)
{
    ...
    int                         nRetCode        = 0;
    KPlayer*                    pPlayer         = NULL;
    DWORD                       dwLeaderMapID   = 0;
    int                         nPopulation     = 0;
    int                         nGroupIndex     = INVALID_TEAM_GROUP;
    DWORD                       dwTeamID        = ERROR_ID;
    KTEAM_MEMBER_LIST::iterator it;
    KTEAM_MEMBER_LIST::iterator itEnd;
#ifdef _SERVER
    KTeam*                  pTeam           = NULL;
#endif

    KG_PROCESS_SUCCESS(!m_bIsFormationSkill);

    nRetCode = IS_PLAYER(pCaster->m_dwID);
    KG_PROCESS_ERROR(nRetCode);

    pPlayer = (KPlayer*)pCaster;

    dwTeamID = pPlayer->GetCurrentTeamID();
    KG_PROCESS_ERROR_RET_CODE(dwTeamID != ERROR_ID, srcNotTeam);
    ...
    KG_PROCESS_ERROR_RET_CODE(pPlayer->m_dwID == pTeam->MemberGroup[nGroupIndex].dwFormationLeader, srcNotFormationLeader);

    it = pTeam->MemberGroup[nGroupIndex].MemberList.begin();
    itEnd = pTeam->MemberGroup[nGroupIndex].MemberList.end();
#else
    KG_PROCESS_ERROR_RET_CODE(
        pPlayer->m_dwID == g_pSO3World->m_TeamClient.m_TeamGroup[nGroupIndex].dwFormationLeader,
        srcNotFormationLeader
    );

    it = g_pSO3World->m_TeamClient.m_TeamGroup[nGroupIndex].MemberList.begin();
    itEnd = g_pSO3World->m_TeamClient.m_TeamGroup[nGroupIndex].MemberList.end();
#endif
    dwLeaderMapID   = pPlayer->m_pScene->m_dwMapID;
    ...
    for (; it != itEnd; ++it)
    {
        ...
        if (pMemberInfo->dwMapID != dwLeaderMapID)
        {
            continue;
        }
        ...
        nRetCode = g_InRange(
            static_cast<KSceneObject*>(pPlayer), 
        ...
        if (nPopulation >= m_nLeastFormationPopulation)
        {
            break;
        }
    }

    KG_PROCESS_ERROR_RET_CODE(nPopulation >= m_nLeastFormationPopulation, srcNotEnoughMember);
...
SKILL_RESULT_CODE KSkill::CheckSelfState(KCharacter *pCaster)
{
    ...
    int               nMoveStateMask      = 0;

    KG_ASSERT_EXIT(pCaster);

    nRetCode = (1 << pCaster->m_eMoveState);

    nMoveStateMask = m_pBaseInfo->nSelfMoveStateMask;
    if (pCaster->m_dwDirectCastMask & m_pBaseInfo->dwCastMask)
    {
        nMoveStateMask = m_pBaseInfo->nSelfBackupMoveStateMask;
    }

    KG_PROCESS_ERROR_RET_CODE(nRetCode & nMoveStateMask, srcMoveStateError);
...
BOOL KSkill::AutoSelectTarget(KCharacter* pCaster)
{
    ...
    KSearchForCharacter::KResultVec::iterator it;
...
int KSkill::GetSkillDisplayInfo(KSKILL_DISPLAY_INFO* pSkillDisplayInfo)
{
    ...
    pSkillDisplayInfo->nCostRage    = m_nCostRage;
...
struct FindTarget
{
    ...
    BOOL operator () (KCharacter* pCharacter)
    {
        ...
        nDistance = g_GetDistance3(
            pCenter->m_nX, pCenter->m_nY, ZPOINT_TO_XYPOINT(pCenter->m_nZ),
...
SKILL_RESULT_CODE KSkill::CastOnTargetLeader(KCharacter* pDisplayCaster, KCharacter* pLogicCaster, const KSKILL_RECIPE_KEY& crRecipeKey)
{
    ...
    dwTeamID = pPlayer->GetCurrentTeamID();
    pTeam = g_pSO3World->m_TeamServer.GetTeam(dwTeamID);
    KGLOG_PROCESS_ERROR_RET_CODE(pTeam, srcNotTeam);
...
SKILL_RESULT_CODE KSkill::CastOnTeamArea(KCharacter* pDisplayCaster, KCharacter* pLogicCaster, const KSKILL_RECIPE_KEY& crRecipeKey)
{
    SKILL_RESULT_CODE   nResult     = srcFailed;
    int                 nRetCode    = 0;
    KPlayer*            pPlayer     = NULL;
    ...
    if (!pTeam)
    {
        ...
        nRetCode = CheckTargetBuff(Target);
        KG_PROCESS_ERROR(nRetCode == srcSuccess);
    ...
    )
    {
        ...
        nRetCode = SkillInRange(
            pDisplayCaster->m_nX, pDisplayCaster->m_nY, ZPOINT_TO_XYPOINT(pDisplayCaster->m_nZ),
            pMember->m_nX, pMember->m_nY, ZPOINT_TO_XYPOINT(pMember->m_nZ),
            m_nAreaRadius
        );
        if (!nRetCode)
            continue;
...
BOOL KSkill::SetupBullet(KSKILL_BULLET *pBullet, KCharacter* pCaster, KTarget &rTarget, const KSKILL_RECIPE_KEY& crRecipeKey)
{
    ...
    pBullet->dwAddDamageByDstMoveStateMask      = pCaster->m_dwAddDamageByDstMoveStateMask;
    pBullet->nAddDamagePercentByDstMoveState    = pCaster->m_nAddDamagePercentByDstMoveState;
    ...
    case sktPhysics:
        ...
        nDenominator = 74 * pCaster->m_nLevel + 320;
        KGLOG_PROCESS_ERROR(nDenominator);

        pBullet->nCriticalStrikePower =
            pCaster->m_nPhysicsCriticalDamagePowerBaseKiloNumRate + KILO_NUM *
            pCaster->m_nPhysicsCriticalDamagePower / nDenominator;
        break;
    case sktSolarMagic:
        ...
        pBullet->nCriticalStrikePower =
            pCaster->m_nSolarCriticalDamagePowerBaseKiloNumRate + KILO_NUM *
            pCaster->m_nSolarCriticalDamagePower / nDenominator;
        break;
    case sktNeutralMagic:
        ...
        pBullet->nCriticalStrikePower =
            pCaster->m_nNeutralCriticalDamagePowerBaseKiloNumRate + KILO_NUM *
            pCaster->m_nNeutralCriticalDamagePower / nDenominator;
        break;
    case sktLunarMagic:
        ...
        pBullet->nCriticalStrikePower =
            pCaster->m_nLunarCriticalDamagePowerBaseKiloNumRate + KILO_NUM *
            pCaster->m_nLunarCriticalDamagePower / nDenominator;
        break;
    case sktPoison:
        ...
        pBullet->nCriticalStrikePower =
            pCaster->m_nPoisonCriticalDamagePowerBaseKiloNumRate + KILO_NUM *
            pCaster->m_nPoisonCriticalDamagePower / nDenominator;
        break;
    case sktLeap:
        break;
    case sktNone:
    ...
    // 计算子弹飞行时间
    nDistance = g_GetDistance3(
        nCasterX, nCasterY, ZPOINT_TO_XYPOINT(nCasterZ), 
        nTargetX, nTargetY, ZPOINT_TO_XYPOINT(nTargetZ)
    );
