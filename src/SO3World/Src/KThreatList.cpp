...
BOOL KSimpThreatList::ModifyThreat(THREAT_TYPE eThreatType, KCharacter* pCharacter, int nThreat)
{
    ...
    KG_PROCESS_ERROR(pCharacter->m_dwID != m_pSelf->m_dwID);

    // 自己已经被移除了，就不要再加仇恨了，否则仇恨表不Active会导致别人出不了战斗。
    KG_PROCESS_SUCCESS(!m_pSelf->m_pScene);

    KGLOG_PROCESS_ERROR(m_pSelf->m_pRegion);

    // 不仇恨已经死了的人。这样，对方死了之后，即使DOT跳也不会进战斗了。
    KG_PROCESS_SUCCESS(pCharacter->m_eMoveState == cmsOnDeath);

    KG_PROCESS_SUCCESS(pCharacter->m_pScene != m_pSelf->m_pScene);
...
void KSimpThreatList::OnCharacterEnterNpcThreatList(KCharacter* pEnterCharacter)
{
    ...
    if (IS_PLAYER(pEnterCharacter->m_dwID) && pNpc->m_dwDropTargetPlayerID != ERROR_ID)
    {
        ...
        dwEnterTeamID = pEnterPlayer->GetCurrentTeamID();
        if (pDropTarget)
            dwDropTeamID  = pDropTarget->GetCurrentTeamID();
        if (
    ...
    if (dwDropTeamID != ERROR_ID)
    {
        ...
        while (cpThreatNode)
        {
            ...
            if (IS_PLAYER(cpThreatNode->pCharacter->m_dwID))
            {
                ...
                DWORD    dwThreatTeamID = pThreatPlayer->GetCurrentTeamID();
                if (
...
void KSimpThreatList::OnPlayerEnterPlayerThreatList(KPlayer* pTarget)
{
    ...
    KG_PROCESS_ERROR(pSelf->m_eCamp != cNeutral && pTarget->m_eCamp != cNeutral);
    KG_PROCESS_ERROR(pSelf->m_eCamp != pTarget->m_eCamp); // 我和他在同一阵营不用开开关
    ...
    if (bRetCode)
    {
        ...
        KG_PROCESS_ERROR(dwTargetID != pTarget->m_dwID);
    }
