...
struct KTraverseScenePlayerFunc
{
    ...
    std::vector<DWORD> vecScenePlayer;
};

BOOL KTraverseScenePlayerFunc::operator()(KPlayer* pPlayer)
{
    assert(pPlayer);
    vecScenePlayer.push_back(pPlayer->m_dwID);
    return true;
}

int KScene::LuaGetAllPlayer(Lua_State* L)
{
    KTraverseScenePlayerFunc TraverseScenePlayerFunc;
    
    TraversePlayer(TraverseScenePlayerFunc);

    Lua_NewTable(L);

    for (size_t i = 0; i < TraverseScenePlayerFunc.vecScenePlayer.size(); i++)
    {
        Lua_PushNumber(L, i + 1);
        Lua_PushNumber(L, TraverseScenePlayerFunc.vecScenePlayer[i]);
...
struct KTraverseSceneNpcFunc
{
    BOOL operator()(KNpc* pNpc);
    std::vector<DWORD> vecSceneNpc;
};

BOOL KTraverseSceneNpcFunc::operator()(KNpc* pNpc)
{
    assert(pNpc);
    vecSceneNpc.push_back(pNpc->m_dwID);
    return true;
}

int KScene::LuaGetAllNpc(Lua_State* L)
{
    KTraverseSceneNpcFunc TraverseSceneNpcFunc;

    TraverseNpc(TraverseSceneNpcFunc);

    Lua_NewTable(L);

    for (size_t i = 0; i < TraverseSceneNpcFunc.vecSceneNpc.size(); i++)
    {
        Lua_PushNumber(L, i + 1);
        Lua_PushNumber(L, TraverseSceneNpcFunc.vecSceneNpc[i]);
...
struct KTraverseSceneDoodadFunc
{
    BOOL operator()(KDoodad* pDoodad);
    std::vector<DWORD> vecSceneDoodad;
};

BOOL KTraverseSceneDoodadFunc::operator()(KDoodad* pDoodad)
{
    assert(pDoodad);
    vecSceneDoodad.push_back(pDoodad->m_dwID);
    return true;
}

int KScene::LuaGetAllDoodad(Lua_State* L)
{
    KTraverseSceneDoodadFunc TraverseSceneDoodadFunc;

    TraverseDoodad(TraverseSceneDoodadFunc);

    Lua_NewTable(L);

    for (size_t i = 0; i < TraverseSceneDoodadFunc.vecSceneDoodad.size(); i++)
    {
        Lua_PushNumber(L, i + 1);
        Lua_PushNumber(L, TraverseSceneDoodadFunc.vecSceneDoodad[i]);
...
int KScene::LuaTimeLimitationBindItemAddItem(Lua_State* L)
{
    int         nRetCode    = false;
    int         nLeftTime   = 0;
    DWORD       dwItemID    = 0;
    ...
    dwOwnerID  = (DWORD)Lua_ValueToNumber(L, 2);
    nLeftTime  = (int)Lua_ValueToNumber(L, 3);
    
    nRetCode = m_TimeLimitationBindItemMgr.AddItem(dwItemID, dwOwnerID, nLeftTime);
    KGLOG_PROCESS_ERROR(nRetCode);
...
int KScene::LuaTimeLimitationBindItemDelItem(Lua_State* L)
{
    int         nRetCode    = false;
    DWORD       dwItemID    = 0;
    ...
    m_TimeLimitationBindItemMgr.DelItem(dwItemID);

Exit0:
    return 0;
}

int KScene::LuaTimeLimitationBindItemAddPlayer(Lua_State* L)
{
    int         nRetCode    = false;
    DWORD       dwItemID    = 0;
    ...
    nRetCode = m_TimeLimitationBindItemMgr.AddPlayer(dwItemID, dwPlayerID);
    KGLOG_PROCESS_ERROR(nRetCode);
...
int KScene::LuaTimeLimitationBindItemRemovePlayer(Lua_State* L)
{
    int         nRetCode    = false;
    DWORD       dwItemID    = 0;
    DWORD       dwPlayerID  = ERROR_ID;
    ...
    nRetCode = m_TimeLimitationBindItemMgr.RemovePlayer(dwItemID, dwPlayerID);
    KGLOG_PROCESS_ERROR(nRetCode);
...
int KScene::LuaTimeLimitationBindItemGetLeftTime(Lua_State* L)
{
    int         nRetCode    = false;
    DWORD       dwItemID    = 0;
    int         nLeftTime   = 0;
    ...
    nLeftTime = m_TimeLimitationBindItemMgr.GetLeftTime(dwItemID);
...
int KScene::LuaTimeLimitationBindItemGetItemOwnerID(Lua_State* L)
{
    int         nRetCode    = false;
    DWORD       dwItemID    = 0;
    DWORD       dwOwnerID   = ERROR_ID;
    ...
    dwOwnerID = m_TimeLimitationBindItemMgr.GetOwner(dwItemID);
Exit0:
    Lua_PushNumber(L, dwOwnerID);
    return 1;
}

int KScene::LuaTimeLimitationBindItemGetAllPlayer(Lua_State* L)
{
    int         nRetCode    = false;
    DWORD       dwItemID    = 0;
    int         nLeftTime   = 0;
    std::vector<DWORD> vecPlayers;
    ...
    nRetCode = m_TimeLimitationBindItemMgr.GetAllPlayer(dwItemID, vecPlayers);
    KGLOG_PROCESS_ERROR(nRetCode);
...
DEFINE_LUA_CLASS_BEGIN(KScene)
    ...
    REGISTER_LUA_FUNC(KScene, TimeLimitationBindItemAddItem)
    REGISTER_LUA_FUNC(KScene, TimeLimitationBindItemDelItem)
    REGISTER_LUA_FUNC(KScene, TimeLimitationBindItemAddPlayer)
    REGISTER_LUA_FUNC(KScene, TimeLimitationBindItemRemovePlayer)
    REGISTER_LUA_FUNC(KScene, TimeLimitationBindItemGetLeftTime)
    REGISTER_LUA_FUNC(KScene, TimeLimitationBindItemGetItemOwnerID)
    REGISTER_LUA_FUNC(KScene, TimeLimitationBindItemGetAllPlayer)
