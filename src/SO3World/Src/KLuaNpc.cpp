...
int KNpc::LuaSetActiveMark(Lua_State* L)
{
    int nRetCode = 0;
    ...
    m_bActiveMark = (BOOL)Lua_ValueToBoolean(L, 1);

    if (m_bActiveMark && m_pRegion)
        m_pRegion->SetActive();
...
int KNpc::LuaCopyThreatList(Lua_State* L)
{
    BOOL                    bResult         = false;
    DWORD                   dwDstID         = 0;
    ...
    dwDstID = (DWORD)Lua_ValueToNumber(L, 1);
    KGLOG_PROCESS_ERROR(dwDstID != m_dwID);
    KGLOG_PROCESS_ERROR(IS_NPC(dwDstID));

    pDstCharacter = g_pSO3World->m_NpcSet.GetObj(dwDstID);
    KGLOG_PROCESS_ERROR(pDstCharacter);
    KG_PROCESS_ERROR(pDstCharacter->m_pScene);
...
int KNpc::LuaSetNpcTeamID(Lua_State* L)
{
    BOOL    bResult         = false;
    BOOL    bRetCode        = false;
    int     nNpcTeamID      = 0;
    ...
    nNpcTeamID = (int)Lua_ValueToNumber(L, 1);
    KGLOG_PROCESS_ERROR(nNpcTeamID >= 0);

    KG_PROCESS_SUCCESS(m_nNpcTeamID == nNpcTeamID);

    if (m_nNpcTeamID)
    {
        bRetCode = m_pScene->m_ThreatLink.DelMember(m_nNpcTeamID, this);
        KGLOG_PROCESS_ERROR(bRetCode);
    }

    m_nNpcTeamID = nNpcTeamID;

    if (m_nNpcTeamID)
    {
        bRetCode = m_pScene->m_ThreatLink.AddMember(m_nNpcTeamID, this);
        KGLOG_PROCESS_ERROR(bRetCode);
    }
...
int KNpc::LuaSayToCamp(Lua_State* L)
{
    BOOL        bResult     = false;
    int         nRetCode    = 0;
    ...
    eCamp = (KCAMP)(int)Lua_ValueToNumber(L, 1);
    pszText = (char*)Lua_ValueToString(L, 2);
    KGLOG_PROCESS_ERROR(pszText);

    nRetCode = SayToCamp(eCamp, pszText);
    KGLOG_PROCESS_ERROR(nRetCode);
...
int KNpc::LuaCopyAITarget(Lua_State* L)
{
    BOOL        bResult         = false;
    DWORD       dwDstID         = 0;
    ...
    case aittSelectTarget:
        pDstCharacter->m_SelectTarget.SetTarget(m_SelectTarget.m_eTargetType, m_SelectTarget.m_dwID);
        break;
    case aittSkillTarget:
        pDstCharacter->m_SelectTarget.SetTarget(m_SkillTarget.m_eTargetType, m_SkillTarget.m_dwID);
        break;
    default:
        KGLOG_PROCESS_ERROR(nTargetType >= 0);
        pDstCharacter->m_AIData.ChoiceTarget[nTargetType].SetTarget(
            m_AIData.ChoiceTarget[nTargetType].m_eTargetType, 
            m_AIData.ChoiceTarget[nTargetType].m_dwID
        );
        break;
    }
...
#ifdef _SERVER
int KNpc::LuaSetBattleZone(Lua_State* L)
{
    int nTopIndex   = 0;
    int nX          = 0;
    int nY          = 0;
    ...
    nRadius = (int)Lua_ValueToNumber(L, 4);
    KGLOG_PROCESS_ERROR(nRadius >= 0);

    nHeight = (int)Lua_ValueToNumber(L, 5);
    KGLOG_PROCESS_ERROR(nHeight >= 0);

    m_SimpThreatList.SetBattleZone(nX, nY, nZ, nRadius, nHeight);
...
DEFINE_LUA_CLASS_BEGIN(KNpc)
    ...
    REGISTER_LUA_DWORD(KCharacter, GuardForceID)
    ...
    REGISTER_LUA_DWORD(KNpc, DropTargetPlayerID)
    ...
    REGISTER_LUA_INTEGER(KCharacter, BattleFieldSide)
    ...
    REGISTER_LUA_INTEGER_READONLY(KCharacter, AITotalDamageValue)
    ...
    REGISTER_LUA_FUNC(KCharacter, SetAIProtectCampMarkAndForceMark)
    ...
    REGISTER_LUA_FUNC(KCharacter, GetSkillTarget)
    ...
    REGISTER_LUA_FUNC(KNpc, SetActiveMark)
	REGISTER_LUA_FUNC(KNpc, SetPatrolPath)
    REGISTER_LUA_FUNC(KNpc, SetNpcTeamID)
    ...
    REGISTER_LUA_FUNC(KNpc, SayToCamp)

    REGISTER_LUA_FUNC(KNpc, GetAISelectSkill)
    REGISTER_LUA_FUNC(KNpc, CopyAITarget)
...
#ifdef _SERVER
    REGISTER_LUA_FUNC(KNpc, SetBattleZone)
#endif
...
DEFINE_LUA_CLASS_BEGIN(KNpcTemplate)
    ...
    REGISTER_LUA_INTEGER(KNpcTemplate, Intensity)
