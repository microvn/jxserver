...
#ifdef _SERVER
    pDoodad->m_nProgressID      = pTemplate->m_nProgressID;
#endif
...
#ifdef _SERVER
    for (int i = 0; i < MAX_DROP_PER_DOODAD; ++i)
    {
        snprintf(szColName, sizeof(szColName), "Drop%d", i + 1);
        szColName[sizeof(szColName) - 1] = '\0';

        bRetCode = piTabFile->GetString(
            nIndex, szColName, m_DefaultDoodadTemplate.m_szDropName[i], fDoodadTemplate.m_szDropName[i], MAX_PATH
        );
	    (void)bRetCode; /*[endgame] tolerant*/
        ...
        bRetCode = piTabFile->GetInteger(
            nIndex, szColName, m_DefaultDoodadTemplate.m_dwDropCount[i], (int*)&fDoodadTemplate.m_dwDropCount[i]
        );
	    (void)bRetCode; /*[endgame] tolerant*/
    }
#endif
