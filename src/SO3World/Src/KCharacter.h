...
// 根据ID判断是否是玩家，玩家ID的高2位为0
#define IS_NPC(ID)    ((ID) & 0x40000000)
...
#define FLOTAGE_COEFFICIENT         512
...
public:
    ...
    DWORD						m_dwGuardForceID;           // 受该势力的守卫保护
	int							m_nLevel;					// 角色等级
    ...
    DECLARE_LUA_DWORD(GuardForceID);
    ...
    BOOL                        m_bPositiveShield;          // 正面效果屏蔽,屏蔽所有正向效果(如加血,Buff)技能
    BOOL                        m_bNegativeShield;          // 负面效果屏蔽,屏蔽所有负向效果(如伤害, Debuff)技能
    ...
    int                         m_nKnockedDownRate;         // 被击倒率
    int                         m_nRepulsedRate;            // 被击滑步率
    int                         m_nPullRate;                // 被拉扯率
    ...
    DECLARE_LUA_BOOL(PositiveShield);
    DECLARE_LUA_BOOL(NegativeShield);
    ...
    int m_nPhysicsCriticalDamagePowerBaseKiloNumRate;     // 增加会心伤害
    int m_nPhysicsCriticalDamagePower;
    int m_nPhysicsCriticalDamagePowerBase;
    int m_nPhysicsCriticalDamagePowerPercent;
    ...
    DECLARE_LUA_INTEGER(PhysicsCriticalDamagePowerBaseKiloNumRate);
    DECLARE_LUA_INTEGER(PhysicsCriticalDamagePower);
    ...
    int m_nSolarCriticalDamagePowerBaseKiloNumRate;     // 增加会心伤害
    int m_nSolarCriticalDamagePower;
    int m_nSolarCriticalDamagePowerBase;
    int m_nSolarCriticalDamagePowerPercent;
    ...
    DECLARE_LUA_INTEGER(SolarCriticalDamagePowerBaseKiloNumRate);
    DECLARE_LUA_INTEGER(SolarCriticalDamagePower);
    ...
    int m_nNeutralCriticalDamagePowerBaseKiloNumRate;     // 增加会心伤害
    int m_nNeutralCriticalDamagePower;
    int m_nNeutralCriticalDamagePowerBase;
    int m_nNeutralCriticalDamagePowerPercent;
    ...
    DECLARE_LUA_INTEGER(NeutralCriticalDamagePowerBaseKiloNumRate);
    DECLARE_LUA_INTEGER(NeutralCriticalDamagePower);
    ...
    int m_nLunarCriticalDamagePowerBaseKiloNumRate;     // 增加会心伤害
    int m_nLunarCriticalDamagePower;
    int m_nLunarCriticalDamagePowerBase;
    int m_nLunarCriticalDamagePowerPercent;
    ...
    DECLARE_LUA_INTEGER(LunarCriticalDamagePower);
    ...
    int m_nPoisonCriticalDamagePowerBaseKiloNumRate;     // 增加会心伤害
    int m_nPoisonCriticalDamagePower;
    int m_nPoisonCriticalDamagePowerBase;
    int m_nPoisonCriticalDamagePowerPercent;
    ...
    DECLARE_LUA_INTEGER(PoisonCriticalDamagePowerBaseKiloNumRate);
    DECLARE_LUA_INTEGER(PoisonCriticalDamagePower);
...
public:
    ...
    // 2. 身上缠绕的.
    // 3. 头上顶着的.
	BYTE    m_byRepresentState[brpTotal];
...
public:
    ...
    // DECLARE_LUA_INTEGER(AITotalDamageValue);
    DWORD getAITotalDamageValue(void) {return m_AIData.nAITotalDamageValue;};
...
public:
    ...
    void RecordPosition(KPOSITION_RECORD* pRecord);
    BOOL ReversePosition(KPOSITION_RECORD* pRecord);
...
public:
    ...
    struct KDAMAGE_CALCULATE_PARAM
    {
        ...
        int          nDamageToLifeForSrcTeam;      // 伤害转化为释放者的小队所有成员的生命
        int          nDamageToManaForSrcTeam;      // 伤害转化为释放者的小队所有成员的内力
...
public:
    ...
    int LuaGetSkillTarget(Lua_State* L);
    ...
    int LuaSetAIProtectCampMarkAndForceMark(Lua_State* L);
