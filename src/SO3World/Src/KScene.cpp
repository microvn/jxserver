...
#endif
    m_TimeLimitationBindItemMgr.Init(this);
...
int KScene::Activate(void)
{
    ...
    m_TimeLimitationBindItemMgr.Activate();
...
BOOL KScene::LoadPartitionTable()
{
	...
	snprintf(szFileName, sizeof(szFileName), "%s/%s/Partition.tab", MAP_DIR, m_szName);
    szFileName[sizeof(szFileName) - 1] = '\0';
...
#ifdef _SERVER
    pNpc->RegisterToViewRegion(NULL, pRegion);
#endif
...
BOOL KScene::AddPlayer(KPlayer* pPlayer)
{
    ...
    pPlayer->RegisterViewObjectByPlayer(NULL, pRegion);
#endif
    ...
    g_pSO3World->m_StatDataServer.UpdateEnterScene(this, pPlayer);
#endif
...
BOOL KScene::RemovePlayer(KPlayer* pPlayer)
{
    ...
    m_TimeLimitationBindItemMgr.OnPlayerLeaveScene(pPlayer->m_dwID);
...
#ifdef _SERVER
void KScene::SetViewObjectRegisterFlag(KRegion* pCenter, BOOL bFlag)
{
    int                 nLeft               = 0;
    int                 nRight              = 0;
    int                 nBottom             = 0;
    ...
    KG_PROCESS_ERROR(pCenter);

    nLeft       = pCenter->m_nRegionX - m_nBroadcastRegion;
    nRight      = pCenter->m_nRegionX + m_nBroadcastRegion;
    nBottom     = pCenter->m_nRegionY - m_nBroadcastRegion;
    nTop        = pCenter->m_nRegionY + m_nBroadcastRegion;
...
// 注册自己到目标player同步表中
struct KRegisterSelfToPlayerViewFunc
{
    KRegisterSelfToPlayerViewFunc(KPlayer* pPlayer, KRegion* pRegion) : m_pSelf(pPlayer), m_pFromRegion(pRegion) {};
    ...
    BOOL operator () (KPlayer* pPlayer)
    {
        ...
        // 如果对方的ViewPoint之前已经能看到我了,就不用再将我注册到目标同步表中去
        if (pPlayer->IsInViewRangeByViewPoint(m_pFromRegion))
            return true;

        InsRet = pPlayer->m_PlayerSyncTable.insert(m_pSelf->m_dwID);
        if (InsRet.second)
            pPlayer->m_PlayerSyncQueue.push_back(KPlayer::KViewObject(m_pSelf->m_dwID, g_pSO3World->m_nGameLoop + VIEW_SYNC_DELAY));
...
struct KPlayerViewFunc
{
    ...
    BOOL operator () (KPlayer* pPlayer)
    {
        ...
        InsRet = m_pSelf->m_PlayerSyncTable.insert(pPlayer->m_dwID);
        if (InsRet.second)
...
struct KNpcViewFunc
{
    ...
    BOOL operator () (KNpc* pNpc)
    {
        ...
        InsRet = m_pSelf->m_NpcSyncTable.insert(pNpc->m_dwID);
        if (InsRet.second)
...
// 将自己注册到目标ViewPoint的拥有者同步表中
struct KViewPointViewFunc
{
    KViewPointViewFunc(KPlayer* pPlayer) : m_pSelf(pPlayer) {};

    BOOL operator () (KPlayer* pPlayer)
    {
        std::pair<KPlayer::KVIEW_SYNC_TABLE::iterator, BOOL> InsRet = 
            pPlayer->m_PlayerSyncTable.insert(m_pSelf->m_dwID);
        if (InsRet.second)
            pPlayer->m_PlayerSyncQueue.push_back(KPlayer::KViewObject(m_pSelf->m_dwID, g_pSO3World->m_nGameLoop));
...
// 调用本函数前应该先调用SetViewObjectRegisterFlag()函数
BOOL KScene::RegisterPlayerToArea(KPlayer* pPlayer, KRegion* pCenter)
{
    BOOL                            bResult             = false;
    int                             nLeft               = 0;
    int                             nRight              = 0;
    int                             nBottom             = 0;
    int                             nTop                = 0;
    KRegisterSelfToPlayerViewFunc   SelfToPlayerViewFunc(pPlayer, pCenter);
    KViewPointViewFunc              ViewPointViewFunc(pPlayer);

    KGLOG_PROCESS_ERROR(pPlayer);
    KGLOG_PROCESS_ERROR(pCenter);

    KG_PROCESS_SUCCESS(pPlayer->m_bStealthy);
    ...
    for (int nX = nLeft; nX <= nRight; nX++)
    {
        for (int nY = nBottom; nY <= nTop; nY++)
        {
            ...
            {
                pRegion->TraversePlayer(SelfToPlayerViewFunc);
                pRegion->TraverseViewPoint(ViewPointViewFunc, pCenter);
            }
        }
    }
...
// 调用本函数前应该先调用SetViewObjectRegisterFlag()函数
BOOL KScene::RegisterAreaObjectToPlayer(KPlayer* pPlayer, KRegion* pCenter)
{
    BOOL                bResult             = false;
    int                 nLeft               = 0;
    int                 nRight              = 0;
    int                 nBottom             = 0;
    int                 nTop                = 0;
    KPlayerViewFunc     PlayerViewFunc(pPlayer);
    KNpcViewFunc        NpcViewFunc(pPlayer);
    KDoodadViewFunc     DoodadViewFunc(pPlayer);
