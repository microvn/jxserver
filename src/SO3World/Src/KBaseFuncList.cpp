...
Exit0:
	...
	int LuaGetLevelUpData(Lua_State* L)
	{
        ...
        Lua_PushString(L, "ParryBaseRate");
        Lua_PushNumber(L, pLevelUpData->nParryBaseRate);
        Lua_SetTable(L, nLevelTable);
        ...
        Lua_PushString(L, "MaxContribution");
        Lua_PushNumber(L, pLevelUpData->nMaxContribution);
        Lua_SetTable(L, nLevelTable);
...
Exit0:
	...
	int LuaGetMapParams(Lua_State* L)
	{
        ...
        Lua_PushNumber(L, pMapParams->nType);
        Lua_PushNumber(L, pMapParams->nMaxPlayerCount);
        Lua_PushNumber(L, pMapParams->nLimitTimes);
        Lua_PushNumber(L, pMapParams->nCampType);
...
Exit0:
    ...
    int LuaGetMapRefreshInfo(Lua_State* L)
    {
        int         nResult         = 0;
        DWORD       dwMapID         = 0;
        KMapParams* pMapParams      = NULL;
        time_t      nGSCurrentTime  = g_pSO3World->m_nBaseTime + (g_pSO3World->m_nGameLoop / GAME_FPS);
        ...
        if (pMapParams->nRefreshCycle > 0)
        {
            nNextRefreshTime = (nGSCurrentTime / pMapParams->nRefreshCycle + 1) * pMapParams->nRefreshCycle;;
        }
        
        Lua_PushNumber(L, pMapParams->nRefreshCycle);
        Lua_PushNumber(L, nNextRefreshTime);
...
#define LUA_FILL_TABLE_INT(L, nTable, Name, Store)      \
    ...
    int LuaGetSkillInfo(Lua_State* L)
    {
        ...
        LUA_FILL_TABLE_INT(L, nTable, CostRage, SkillDisplayInfo);
...
Exit0:
	...
	int LuaSetUserPreferences(Lua_State* L)
	{
		...
		while (*pcszEncodeType)
		{
			...
			case 'D':
				{
					DWORD dwData = (DWORD)(int)Lua_ValueToNumber(L, nCurrent);
...
Exit0:
	...
	int LuaGetUserPreferences(Lua_State* L)
	{
		...
		case 'd':
		case 'D':
			Lua_PushNumber(L, (int)(*((DWORD *)pData)));
			break;
		case 'n':
		case 'N':
...
Exit0:
	...
	int LuaResetGameworld(Lua_State* L)
	{
        ...
        g_pSO3World->m_AuctionClient.ClearAll();
        g_pSO3World->m_MailClient.Reset();
        g_pSO3World->m_ShopCenter.ClearShop();
        g_pSO3World->m_MentorClient.Clear();
...
int LuaSendSystemMail(Lua_State* L)
{
    ...
    KGLOG_PROCESS_ERROR(nTopIndex <= 9);
    ...
    if (nTopIndex >= 8)
    {
        ...
        nStackNum   = (int)Lua_ValueToNumber(L, 8);
        ...
        nRetCode = pItem->GetBinaryData(&uItemDataLen, byItemData, sizeof(byItemData));
        KGLOG_PROCESS_ERROR(nRetCode);

        if (nTopIndex == 9)
        {
            nItemPrice = (int)Lua_ValueToNumber(L, 9);
            KGLOG_PROCESS_ERROR(nItemPrice >= 0);
        }
    }

    uMailLen = sizeof(KMail) + uTextLen + uItemDataLen;
    KGLOG_PROCESS_ERROR(uMailLen <= sizeof(byBuffer));

    pMail = (KMail*)byBuffer;
    ...
    strncpy(pMail->szTitle, pszMailTitle, sizeof(pMail->szTitle));
    pMail->szTitle[sizeof(pMail->szTitle) - 1] = '\0';

    strncpy(pMail->szSenderName, pszSender, sizeof(pMail->szSenderName));
    ...
    memcpy(pMail->byData, pszText, uTextLen);
    ...
    g_RelayClient.DoSendMailRequest(0, pszReciever, pMail, uMailLen);
...
int LuaSendGlobalMail(Lua_State* L)
{
    ...
    g_RelayClient.DoSendGlobalMailRequest(pMail, uMailLen, nLifeTime);
...
int LuaGetMasterSkillList(Lua_State* L)
{
	...
	for (LearnSkillArray::iterator it = KungfuArray.begin(); it != KungfuArray.end(); ++it)
	{
		...
		if (nShowGroup)
		{
			...
			pInfo = g_pSO3World->m_SkillManager.GetSkillLearningInfo(dwMastrID, it->dwID, nKunfuLevel ? nKunfuLevel : 1);
			nRLevel = 0;
			if(pInfo)
			...
			for (std::vector<KSKILL_ID_LEVEL>::iterator itS = it->aSkill.begin();  itS != it->aSkill.end(); ++itS)
			{
				...
				Lua_PushString(L, "nRLevel");
				pInfo = g_pSO3World->m_SkillManager.GetSkillLearningInfo(dwMastrID, itS->dwSkillID, nSkillLevel ? nSkillLevel : 1);
				nRLevel = 0;
				if(pInfo)
					nRLevel = pInfo->nRequirePlayerLevel;
				Lua_PushNumber(L, nRLevel);
...
int LuaPeekOtherPlayerEquip(Lua_State* L)
{
    ...
    {
        pItem = pTarget->m_ItemList.PickUpItem(ibEquip, i);
        if (pItem)
        {
            g_pSO3World->m_ItemManager.FreeItem(pItem);
            pItem = NULL;
...
{
    g_pSO3World->m_ApexClient.EndApexClient();
...
#ifdef _SERVER
int LuaSetRouteNodeCamp(Lua_State* L)
{
    int             nResult     = 0;
    int             nRetCode    = 0;
    int             nRouteNode  = 0;
    KCAMP           eCamp       = cNeutral;
    ...
    eCamp       = (KCAMP)(int)Lua_ValueToNumber(L, 2);

    pNode       = g_pSO3World->m_RoadManager.GetNodeInfo(nRouteNode);
    KGLOG_PROCESS_ERROR(pNode);

    KGLOG_PROCESS_ERROR(eCamp >= cNeutral && eCamp <= cEvil);

    g_RelayClient.DoSyncRoadTrackInfo(nRouteNode, eCamp, pNode->bEnable);
...
int LuaEnableRouteNode(Lua_State* L)
{
    int             nTopIndex       = 0;
    int             nRouteNodeID    = 0;
    BOOL            bEnable         = false;
    ...
    nRouteNodeID  = (int)Lua_ValueToNumber(L, 1);
    bEnable       = Lua_ValueToBoolean(L, 2);

    pNode       = g_pSO3World->m_RoadManager.GetNodeInfo(nRouteNodeID);
    KGLOG_PROCESS_ERROR(pNode);

    g_RelayClient.DoSyncRoadTrackInfo(nRouteNodeID, pNode->eCamp, bEnable);
...
int LuaCalculateTrackCost(Lua_State* L)
{
    ...
    bResult = true;
Exit0:
    Lua_PushBoolean(L, bResult);    // �Ƿ��н��
    Lua_PushNumber(L, nCost);       // �����ʲô
    return 2;
}
...
struct KFindRoadNodeByCity
{
    ...
    DWORD m_dwCityID;
    std::vector<int> m_vecRes;
    BOOL operator () (KROUTE_NODE* pRouteNode)
    {
        if (pRouteNode->dwCityID == m_dwCityID)
        {
            m_vecRes.push_back(pRouteNode->nNodeID);
        }
        return true;
    }
};

int LuaGetRoadNodeInfoByCity(Lua_State* L)
{
    int                 nResult   = 0;
    int                 nTopIndex = 0;
    int                 nTable    = 0;
    int                 nIndex    = 1;
    DWORD               dwCityID  = 0;
    KFindRoadNodeByCity Func;

    nTopIndex = Lua_GetTopIndex(L);
    KGLOG_PROCESS_ERROR(nTopIndex == 1);

    dwCityID = (DWORD)Lua_ValueToNumber(L, 1);

    Func.m_dwCityID = dwCityID;

    g_pSO3World->m_RoadManager.TraverseNode(Func);

    Lua_NewTable(L);
    nTable = Lua_GetTopIndex(L);

    for (std::vector<int>::iterator it = Func.m_vecRes.begin(); it != Func.m_vecRes.end(); ++it)
    {
        int             nSubTable   = 0;
        KROUTE_NODE*    pNode       = NULL;
        ...
        Lua_PushString(L, "ID");
        Lua_PushNumber(L, pNode->nNodeID);
        Lua_SetTable(L, nSubTable);

        Lua_PushString(L, "Camp");
        Lua_PushNumber(L, (int)pNode->eCamp);
        Lua_SetTable(L, nSubTable);

        Lua_PushString(L, "bEnable");
        Lua_PushBoolean(L, pNode->bEnable);
        Lua_SetTable(L, nSubTable);
...
int LuaGetMentorCache(Lua_State* L)
{
    int nResult     = 0;
    int nTopIndex   = 0;
...
int LuaGetMailClient(Lua_State* L)
{
    int nResult     = 0;
    int nTopIndex   = 0;
    ...
    nResult = g_pSO3World->m_MailClient.LuaGetObj(L);
Exit0:
    return nResult;
}
#endif // _CLIENT
...
#ifdef _CLIENT
int LuaApplyAchievementData(Lua_State* L)
{
    int     nTopIndex   = 0;
    DWORD   dwPlayerID  = ERROR_ID;
    ...
    if (dwPlayerID != g_pSO3World->m_dwClientPlayerID)
    {
        BOOL        bRetCode        = false;
        KPlayer*    pTarget         = NULL;
        KPlayer*    pClientPlayer   = NULL;
    ...
    g_PlayerClient.DoApplyAchievementDataRequest(dwPlayerID);
Exit0:
    return 0;
}
#endif // _CLIENT
...
int LuaGetAchievementInfo(Lua_State* L)
{
    ...
    pInfo = (KACHIEVEMENT_INFO*)g_pSO3World->m_Settings.m_AchievementInfoList.GetInfo(nAchievementID);
    KGLOG_PROCESS_ERROR(pInfo);

    Lua_PushNumber(L, pInfo->nTriggerVal);
    Lua_PushNumber(L, pInfo->nPoint);
    Lua_PushNumber(L, pInfo->nExp);
    Lua_PushNumber(L, pInfo->byPrefix);
    Lua_PushNumber(L, pInfo->byPostfix);
...
#ifdef _SERVER
int LuaGetSceneByMapIDAndIndex(Lua_State* L)
{
    int         nResult     = 0;
    int         nTopIndex   = 0;
    DWORD       dwID        = 0;
    ...
    pScene = g_pSO3World->GetScene(dwID, nCopyIndex);
    KG_PROCESS_ERROR(pScene);

    KG_PROCESS_ERROR(pScene->m_eSceneState == ssNormal);
...
int LuaUpdateStatData(Lua_State* L)
{
    int         nTopIndex   = 0;
    int         nValue      = 0;
    size_t      uNameLen    = 0;
    ...
    g_pSO3World->m_StatDataServer.Update(pszName, nValue);
...
int LuaGetZoneChargeFlag(lua_State* L)
{
    ...
    lua_pushboolean(L, g_pSO3World->m_bZoneChargeFlag);
    nResult = 1;
Exit0:
    return nResult;
}
...
int LuaReloadAllScripts(Lua_State* L)
{
    int nTopIndex = 0;
    ...
    g_pSO3World->m_ScriptCenter.DoReloadAll();
...
{
    g_pSO3World->m_ScriptCenter.StatBegin();
...
{
    g_pSO3World->m_ScriptCenter.StatEnd();
...
int LuaDumpAIRuntimeStat(Lua_State* L)
{
    g_pSO3World->m_AIManager.LogAIRuntimeStat();

    return 0;
}

int LuaStartAIRuntimeStat(Lua_State* L)
{
    memset(g_pSO3World->m_AIManager.m_ActionRunTimeStatisticsArray, 0, sizeof(g_pSO3World->m_AIManager.m_ActionRunTimeStatisticsArray));
    g_pSO3World->m_AIManager.m_NpcAIRunTimeStatisticsMap.clear();

    g_pSO3World->m_AIManager.m_bLogAIRuntimeStat = true;

    return 0;
}

int LuaStopAIRuntimeStat(Lua_State* L)
{
    g_pSO3World->m_AIManager.LogAIRuntimeStat();

    g_pSO3World->m_AIManager.m_bLogAIRuntimeStat = false;
...
int LuaAddFilterText(Lua_State* L)
{
    BOOL        bRetCode        = 0;
    int         nTopIndex       = 0;
    const char* pszFilterText   = NULL;
    const char* pszPos          = NULL;
    int         nTextLen        = 0;
    int         nLine           = 0;
    char        szTempText[MAX_TEXT_LEN];

    nTopIndex = Lua_GetTopIndex(L);
    KGLOG_PROCESS_ERROR(nTopIndex == 1);

    pszFilterText = Lua_ValueToString(L, 1);
    KGLOG_PROCESS_ERROR(pszFilterText);

    pszPos = pszFilterText;

    while (true)
    {
        if (*pszPos == '\n' || *pszPos == '\0')
        {
            nTextLen = (int)(pszPos - pszFilterText);
            if (nTextLen > 0 && *(pszPos - 1) == '\r')
            {
                nTextLen--;
            }

            KGLOG_PROCESS_ERROR(nTextLen < MAX_TEXT_LEN);

            strncpy(szTempText, pszFilterText, nTextLen);
            szTempText[nTextLen] = '\0';

            nLine++;

            bRetCode = g_pSO3World->m_piTextFilter->AddFilterText(szTempText);
            if (!bRetCode)
            {
                KGLogPrintf(KGLOG_ERR, "[Lua] Add filter text failed at line %d\n", nLine);
            }
...
int LuaGetGameCardClient(Lua_State* L)
{
    return g_pSO3World->m_GameCardClient.LuaGetObj(L);
}

int LuaGetGameCardInfoList(Lua_State* L)
{
    return g_pSO3World->m_Settings.m_GameCardInfoList.LuaGetInfoList(L);
}

int LuaApplyCharacterThreatRankList(Lua_State* L)
{
    int     nTopIndex       = 0;
    DWORD   dwCharacterID   = ERROR_ID;
    ...
    g_PlayerClient.DoApplyCharacterThreatRankList(dwCharacterID);
...
Exit0:
	...
	KLuaFunc BaseFuncList[] = 
	{
        ...
        {"LoadGameCardClient",      Luna<KGameCardClient>::Register},
        {"LoadMailClient",          Luna<KMailClient>::Register},
		{"LoadMailInfo",			Luna<KMailInfo>::Register},
#endif

#ifdef _SERVER
        {"LoadMentorCache",         Luna<KMentorCache>::Register},
#endif
        ...
        {"GetMapRefreshInfo",       LuaGetMapRefreshInfo},
...
#ifdef _SERVER
        {"SetRouteNodeCamp",        LuaSetRouteNodeCamp},
        {"EnableRouteNode",         LuaEnableRouteNode},
#endif
        ...
        {"GetRoadNodeInfoByCity",   LuaGetRoadNodeInfoByCity},
        ...
        {"GetMailClient",               LuaGetMailClient},
#endif
...
#ifdef _CLIENT
        {"ApplyAchievementData",        LuaApplyAchievementData},
#endif // _CLIENT
        {"GetAchievementInfo",          LuaGetAchievementInfo},

#ifdef _SERVER
        {"GetSceneByMapIDAndIndex",     LuaGetSceneByMapIDAndIndex},
        {"UpdateStatData",        		LuaUpdateStatData},
        {"GetZoneChargeFlag",           LuaGetZoneChargeFlag},

        {"ReloadAllScripts",            LuaReloadAllScripts},
        {"ScriptStatBegin",             LuaScriptStatBegin},
        {"ScriptStatEnd",               LuaScriptStatEnd},
        {"DumpAIRuntimeStat",           LuaDumpAIRuntimeStat},
        {"StartAIRuntimeStat",          LuaStartAIRuntimeStat},
        {"StopAIRuntimeStat",           LuaStopAIRuntimeStat},
#endif

#ifdef _CLIENT
        {"AddFilterText",               LuaAddFilterText},
        {"GetGameCardClient",           LuaGetGameCardClient},
        {"GetGameCardInfoList",         LuaGetGameCardInfoList},

        {"ApplyCharacterThreatRankList", LuaApplyCharacterThreatRankList},
#endif // _CLIENT
    };
