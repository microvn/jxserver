...
int KPlayer::LuaGetContainType(Lua_State* L)
{
	...
	dwBoxIndex = (int)Lua_ValueToNumber(L, 1);
    
    pBox = m_ItemList.GetInventory(dwBoxIndex);
    KGLOG_PROCESS_ERROR(pBox);

	pBox->GetContainItemType(&nGenerType, &nSubType);
...
int KPlayer::LuaExchangeItem(Lua_State* L)
{
	...
	KGLOG_PROCESS_ERROR(dwDestBox != ibInvalid); // �����ɾ����Ʒ��
...
int KPlayer::LuaDestroyItem(Lua_State* L)
{
    ...
    g_LogClient.LogPlayerItemChange(this, ITEM_OPERATION_TYPE_OUT, &ItemLogInfo, "script destroy");
#endif 
...
int KPlayer::LuaGetItemByIndex(Lua_State* L)
{
	...
	bRetCode = m_ItemList.GetItemPos(dwTab, dwIndex, dwBox, dwX);
	KG_PROCESS_ERROR(bRetCode);
...
int KPlayer::LuaGetEquipSetItemCount(Lua_State* L)
{
	int nCount = 0;
	DWORD dwSetID = 0;
	KGLOG_PROCESS_ERROR(Lua_GetTopIndex(L) == 1);

	dwSetID = (DWORD)(int)Lua_ValueToNumber(L, 1);
	nCount = m_ItemList.GetEquipedSetNum(dwSetID);
...
#ifdef _SERVER
int KPlayer::LuaHaveItem(Lua_State* L)
{
    ...
    bRetCode = m_ItemList.GetItemPos(dwItemID, dwBox, dwX);
    KG_PROCESS_ERROR(bRetCode);

    bHaveItem = true;
Exit0:
    Lua_PushBoolean(L, bHaveItem);
    return 1;
}

int KPlayer::LuaCostSingleItemByID(Lua_State* L)
{
    BOOL    bRetCode    = false;
    DWORD   dwItemID    = 0;
    DWORD   dwBox       = ibInvalid;
    ...
    bRetCode = m_ItemList.CostSingleItem(dwBox, dwX, 1);
	KGLOG_PROCESS_ERROR(bRetCode);
...
int KPlayer::LuaAddItem(Lua_State* L)
{
    ...
    if (nStackNum > 0 && pItem->IsStackable())
    {
	    pItem->m_nStackNum = nStackNum;
	    if (pItem->m_nStackNum > pItem->m_nMaxStackNum)
        {
    ...
    {
        pItem->m_nCurrentDurability = nStackNum;
    }

    if (nBookRecipeID > 0)
    {
        KGLOG_PROCESS_ERROR(pItem->m_Common.nGenre == igBook);
        pItem->m_nCurrentDurability = nBookRecipeID; // �����;ö�����ʾĳһ����
    }
...
Exit1:
    g_LogClient.LogPlayerItemChange(this, ITEM_OPERATION_TYPE_IN, &ItemLogInfo, "get from script");
...
int KPlayer::LuaCostItemInAllPackage(Lua_State* L)
{
    BOOL  bResult       = false;
    BOOL  bRetCode      = false;
    DWORD dwType	    = 0;
    ...
    bRetCode = m_ItemList.CostItemInAllBox(dwType, dwIndex, dwAmount, nBookRecipeID);
    KGLOG_PROCESS_ERROR(bRetCode);
...
int KPlayer::LuaAddMoney(Lua_State* L)
{
    ...
    g_LogClient.LogPlayerMoneyChange(nMoney, this, "script add money");

    g_pSO3World->m_StatDataServer.UpdateMoneyStat(this, nMoney, "OTHER");
...
int KPlayer::LuaAddContribution(Lua_State* L)
{
    ...
    nOldContribution = m_nContribution;
    bRetCode = AddContribution(nAddContribution);
    KGLOG_PROCESS_ERROR(bRetCode);
    nNewContribution = m_nContribution;

    if (nNewContribution != nOldContribution)
        g_pSO3World->m_StatDataServer.UpdateContributeStat(this, nNewContribution - nOldContribution, "OTHER");
...
int KPlayer::LuaGetTotalGameTime(Lua_State* L)
{
    Lua_PushNumber(L, (int)(m_nTotalGameFrame / GAME_FPS));
    return 1;
}

int KPlayer::LuaGetCreateTime(Lua_State* L)
{
    Lua_PushNumber(L, (int)m_nCreateTime);
    return 1;
}
#endif	//_SERVER
...
int KPlayer::LuaAddExp(Lua_State* L)
{
    ...
    g_pSO3World->m_StatDataServer.UpdateExpStat(this, nExpIncrement, "OTHER");
...
int KPlayer::LuaOpenBook(Lua_State* L)
{
    ...
    if (g_pGameWorldUIHandler)
    {
        ...
        UIParam.dwRecipeID = pBook->m_nCurrentDurability; // ���ߵ��;����������������ı���
...
#ifdef _SERVER
int	KPlayer::LuaForgetRecipe(Lua_State* L)
{
    int		nTopIndex	= 0;
    DWORD	dwCraftID	= 0;
    ...
    m_RecipeList.ForgetRecipe(dwCraftID);
...
int KPlayer::LuaGetProfession(Lua_State* L)
{
	...
	for (dwIndex = 0; dwIndex < dwSize; dwIndex++)
	{
        ...
        DWORD dwAdjustLevel         = m_ProfessionList.GetProfessionAdjustLevel(dwProfessionID);
        ...
        Lua_PushString(L, "AdjustLevel");
        Lua_PushNumber(L, dwAdjustLevel);
        Lua_SetTable(L, nSubTable);
...
int KPlayer::LuaGetProfessionAdjustLevel(Lua_State* L)
{
    int     nResult         = 0;
    int     nTopIndex       = 0;
    DWORD   dwProfessionID  = 0;
    DWORD   dwAdjustLevel   = 0;
    ...
    dwAdjustLevel = m_ProfessionList.GetProfessionAdjustLevel(dwProfessionID);
    Lua_PushNumber(L, dwAdjustLevel);
...
int KPlayer::LuaFinishQuest(Lua_State* L)
{
    ...
    nRetCode = rTarget.SetTarget((TARGET_TYPE)nTargetType, dwTargetID);
    KGLOG_PROCESS_ERROR(nRetCode);

	KGLOG_PROCESS_ERROR(nTargetType == ttDoodad || nTargetType == ttNpc);

#ifdef _SERVER
	nRetCode = m_QuestList.Finish(dwQuestID, rTarget, nPresentChoice1, nPresentChoice2);
    ...
    nRetCode = m_QuestList.CanFinish(pQuestInfo, &rTarget);
    if (nRetCode != qrcSuccess)
    {
        if (g_pGameWorldUIHandler)
        {
            KUIEventQuestRespond UIParam;
            UIParam.nQuestRespond = nRetCode;
            UIParam.dwQuestID     = dwQuestID;
...
{
    SHARE_QUEST_RESULT_CODE nResult       = sqrcFailed;
    BOOL                    bRetCode      = false;
    int                     nQuestIndex   = 0;
    DWORD                   dwQuestID     = 0;
    KQuestInfo*             pQuestInfo    = NULL;
	...
	KG_PROCESS_ERROR_RET_CODE(pQuestInfo->bShare, sqrcQuestCannotShare);    
    KG_PROCESS_ERROR_RET_CODE(m_dwTeamID != ERROR_ID, sqrcNotInTeam);

    g_PlayerClient.DoShareQuest(nQuestIndex);
#endif

	nResult = sqrcSuccess;
Exit0:
#ifdef _CLIENT
    if (nResult != sqrcSuccess)
    {
        if (g_pGameWorldUIHandler)
        {
            KUIEventShareQuestResult UIEvent;
            
            UIEvent.nResultCode     = nResult;
            UIEvent.dwDestPlayerID  = ERROR_ID;
            UIEvent.dwQuestID       = dwQuestID;

            g_pGameWorldUIHandler->OnShareQuestResult(UIEvent);
        }
    }
#endif

	Lua_PushBoolean(L, (nResult == sqrcSuccess));
	return 1;
}
...
int KPlayer::LuaForceFinishQuest(Lua_State* L)
{
	...
	bRetCode = m_QuestList.Finish(dwQuestID, target, 0, 4, true); //target����û���õ�,���Բ��ó�ʼ��
...
int KPlayer::LuaRegisterQuestAssistance(Lua_State* L)
{
    ...
    nRetCode = m_QuestList.RegisterTeamAssistance(dwQuestID);
    KG_PROCESS_ERROR(nRetCode);
...
{
	std::map<DWORD, std::vector<int> > QuestClassMap;
	for (int nQuestIndex = 0; nQuestIndex < MAX_ACCEPT_QUEST_COUNT; nQuestIndex++)
	{
		...
		if (dwQuestID)
		{
			...
			if (pQuestInfo)
            {
                if (pQuestInfo->dwClassID)
                {
				    QuestClassMap[pQuestInfo->dwClassID].push_back(nQuestIndex);
                }
            }
		}
	}
	...
	Lua_NewTable(L);
	for (std::map<DWORD, std::vector<int> >::iterator itMap = QuestClassMap.begin(); itMap != QuestClassMap.end(); ++itMap)
	{
		std::vector<int> &raQuestIndex = (*itMap).second;
		Lua_PushNumber(L, (*itMap).first);
		Lua_NewTable(L);
		
		int nIndex = 1;
		for (std::vector<int>::iterator itIndex = raQuestIndex.begin(); itIndex != raQuestIndex.end(); ++itIndex)
		{
			Lua_PushNumber(L, nIndex);
			Lua_PushNumber(L, *(itIndex));
			Lua_SetTable(L, -3);
			nIndex++;
		}
		Lua_SetTable(L, -3);
...
int KPlayer::LuaGetQuestReputationInfo(Lua_State* L)
{
    DWORD       dwQuestID       = 0;
    KQuestInfo* pQuestInfo      = NULL;
    BOOL        bReputeMatch    = true; 
    ...
    if (!pQuestInfo->bRequireReputeAll)
    {
        ...
        for (int i = 0; i < QUEST_PARAM_COUNT; i++)
        {
            ...
            nReputeLevel = m_ReputeList.GetReputeLevel(pQuestInfo->byRequireForceID[i]);
    ...
    if (pQuestInfo->bRequireReputeAll || !bReputeMatch)
    {
        Lua_NewTable(L);
        ...
        for (int i = 0; i < QUEST_PARAM_COUNT; i++)
        {
            ...
            nReputeLevel = m_ReputeList.GetReputeLevel(pQuestInfo->byRequireForceID[i]);
            if (nReputeLevel < pQuestInfo->byReputeLevelMin[i])
            {
                Lua_PushNumber(L, pQuestInfo->byRequireForceID[i]);
                Lua_PushString(L, "low");
                Lua_SetTable(L, -3);  
            }
            else if (nReputeLevel > pQuestInfo->byReputeLevelMax[i])
            {
                Lua_PushNumber(L, pQuestInfo->byRequireForceID[i]);
...
int KPlayer::LuaGetQuestExpAttenuation(Lua_State* L)
{
    ...
    nExp = pQuestInfo->nPresentExp;

	bRetCode = m_QuestList.GetQuestRepeatCutPercent(pQuestInfo, &nRepeatCutPercent);
    KGLOG_PROCESS_ERROR(bRetCode);

    bRetCode = m_QuestList.GetQuestLevelCutPercent(pQuestInfo, &nLevelCutPercent);
    KGLOG_PROCESS_ERROR(bRetCode);
...
int KPlayer::_LuaGetUnAcceptQuestTraceInfo(Lua_State* L)
{
	DWORD dwQuestID = 0;
	int nIndex = 1;
	BOOL bHave = FALSE;
	...
	if (pQuestInfo->nFinishTime)
	{
		...
		Lua_PushNumber(L, pQuestInfo->nFinishTime);
		Lua_SetTable(L, -3);
	}
	...
	for (int i = 0; i < QUEST_PARAM_COUNT; ++i)
	{
		if (pQuestInfo->dwKillNpcTemplateID[i])
		{
			...
			Lua_PushString(L, "need");
			Lua_PushNumber(L, pQuestInfo->wKillNpcAmount[i]);
			Lua_SetTable(L, -3);
			...
			Lua_PushString(L, "template_id");
            Lua_PushNumber(L, pQuestInfo->dwKillNpcTemplateID[i]);
			Lua_SetTable(L, -3);
	...
	{
		DWORD dwTabType = pQuestInfo->byEndRequireItemType[i];
		DWORD dwTabIndex = pQuestInfo->wEndRequireItemIndex[i];
		if (dwTabType && dwTabIndex)
		{
			KItemInfo * pItemInfo = g_pSO3World->m_ItemManager.GetItemInfo(dwTabType, dwTabIndex);
			if (pItemInfo)
			{
				BOOL bHaveItem = FALSE;
				int nBookRecipeID = -1;
				...
				else
				{
					for (int j = 0; j < i; ++j)
					{
						if (dwTabType == pQuestInfo->byEndRequireItemType[j] && dwTabIndex == pQuestInfo->wEndRequireItemIndex[j])
						{
							bHaveItem = TRUE;
							break;
						}
				...
				if (!bHaveItem)
				{
					...
					Lua_PushString(L, "need");
					Lua_PushNumber(L, pQuestInfo->wEndRequireItemAmount[i]);
					Lua_SetTable(L, -3);

					Lua_PushString(L, "have");
					Lua_PushNumber(L, m_ItemList.GetItemTotalAmount(dwTabType, dwTabIndex, nBookRecipeID));
					Lua_SetTable(L, -3);

					Lua_PushString(L, "type");
					Lua_PushNumber(L, dwTabType);
					Lua_SetTable(L, -3);

					Lua_PushString(L, "index");
					Lua_PushNumber(L, dwTabIndex);
					Lua_SetTable(L, -3);
...
int KPlayer::LuaGetQuestTraceInfo(Lua_State* L)
{
	...
	if (nQuestIndex < 0)
	{
		return _LuaGetUnAcceptQuestTraceInfo(L);
	}
...
int KPlayer::LuaSetQuestValue(Lua_State* L)
{
    ...
    KGLOG_PROCESS_ERROR(nRetCode && "nQuestIndex must be number!");
...
int KPlayer::LuaGetFreeRoom(Lua_State* L)
{
    ...
    {
        pBox->GetContainItemType(&nBoxGenerType, &nBoxSubType);
    ...
    dwPos = m_ItemList.GetEmptyPos(dwBoxIndex);
	KG_PROCESS_ERROR(dwPos != UINT_MAX);
    
    Lua_PushNumber(L, dwPos);
...
int KPlayer::LuaGetItemCDProgress(Lua_State* L)
{
    ...
    KGLOG_PROCESS_ERROR(nParamCount == 1 || nParamCount == 2 || nParamCount == 3);
	...
	case 1:
		{   
			...
			if (
                pItem->m_Common.nGenre == igEquipment && pItem->m_nCurrentDurability == 0 && 
				pItem->m_Common.nSub != estArrow && pItem->m_Common.nSub != estPackage && pItem->m_Common.nSub != estAmulet && 
				pItem->m_Common.nSub != estRing && pItem->m_Common.nSub != estPendant && pItem->m_Common.nSub != estWaistExtend &&
				pItem->m_Common.nSub != estBackExtend && pItem->m_Common.nSub != estHorse
            )
			{
				bBroken = true;
			}
	...
	case 2:
		{
            DWORD   dwBox   = (DWORD)Lua_ValueToNumber(L, 1);
            DWORD   dwX     = (DWORD)Lua_ValueToNumber(L, 2);
            KItem*  pItem   = NULL;

            pItem = m_ItemList.GetItem(dwBox, dwX);
			KG_PROCESS_ERROR(pItem);
            
			if (pItem->m_Common.nGenre == igEquipment && pItem->m_nCurrentDurability == 0 && 
				pItem->m_Common.nSub != estArrow && pItem->m_Common.nSub != estPackage && pItem->m_Common.nSub != estAmulet && 
				pItem->m_Common.nSub != estRing && pItem->m_Common.nSub != estPendant && pItem->m_Common.nSub != estWaistExtend &&
				pItem->m_Common.nSub != estBackExtend && pItem->m_Common.nSub != estHorse)
			{
	...
	case 3:
		{
            ...
            dwTabIndex  = (DWORD)Lua_ValueToNumber(L, 3);
		}
		break;
	default:
		break;
    ...
    dwItemCoolDownID    = pItemInfo->dwCoolDownID;
    dwSkillID           = pItemInfo->dwSkillID;
    dwSkillLevel        = pItemInfo->dwSkillLevel;
    
    if (dwItemCoolDownID != 0)
    {
        bIsCooldown = true;
    }

    bRetCode = m_TimerList.GetTimerData(dwItemCoolDownID, nTimerInterval, nTimerEnd);
    if (bRetCode && nTimerEnd > nEndFrame)
    {
        nEndFrame       = nTimerEnd;
	...
	for (int i = 0; i < MAX_SKILL_COOL_DOWN_TIMER; i++)
	{
		...
		bRetCode = m_TimerList.GetTimerData(dwCooldownID, nTimerInterval, nTimerEnd);
		if (!bRetCode)
			continue;
...
int KPlayer::LuaGetSkillCDProgress(Lua_State* L)
{
	...
	KG_PROCESS_ERROR(!pSkill->m_pBaseInfo->bIsPassiveSkill); //�������ܲ���ʾCD

    if (pSkill->m_pBaseInfo->bIsMountable) // ������ڹ�����CD�Ĺ������⴦��
    {
        DWORD   dwCooldownID    = pSkill->m_dwCoolDownID[OTHER_SCHOOL_KUNGFU_CD_INDEX]; // �Ͳ߻��涨����2��CD��ʱ�����������ڣ�3���������ɼ�
        int     nTimerEnd       = 0;
        int     nTimerInterval  = 0;
        KSkill* pKongfu         = NULL;
...
int KPlayer::LuaClearCDTime(Lua_State* L)
{
    int   nTopIndex     = 0;
    DWORD dwCooldownID  = 0;
    ...
    m_TimerList.ClearTimer(dwCooldownID);
...
int KPlayer::LuaUseItem(Lua_State* L)
{
    ...
    eRetCode = m_ItemList.CanUseItem(pItem, dwBox, &UseItemInfo, Target);
...
int KPlayer::LuaGetItemPos(Lua_State* L)
{
	...
	nTopIndex = Lua_GetTopIndex(L);
	KGLOG_PROCESS_ERROR(nTopIndex == 1 || nTopIndex == 2 || nTopIndex == 3);

	switch (nTopIndex)
	{
	case 1:
        dwItemID = (DWORD)Lua_ValueToNumber(L, 1);
		nRetCode = m_ItemList.GetItemPos(dwItemID, dwBox, dwX);
		KG_PROCESS_ERROR(nRetCode);
		break;
	...
	case 2:
		...
		nRetCode = m_ItemList.GetItemPos(dwTabType, dwIndex, dwBox, dwX);
		KG_PROCESS_ERROR(nRetCode);
		break;
...
int KPlayer::IsEquiped(KCustomEquipInfo* pInfo)
{
    ...
    for (DWORD i = eitMeleeWeapon; i < eitTotal; ++i)
	{
		...
		if (
			...
			pItem->m_GenParam.dwIndex == pInfo->dwID &&
			(pItem->m_nMaxDurability == 0 || pItem->m_nCurrentDurability > 0)
		)
			return true;
	}
...
int KPlayer::LuaSearchForPartyMember(Lua_State* L)
{
    ...
    if (dwLastResult != 0)
    {
        KSearchForTeamMember::KResultVec::iterator it;
        for (it = Tactic.m_Result.begin(); it != Tactic.m_Result.end(); ++it)
        {
            ...
            else
            {
                ...
                {
                    pTeammember = Tactic.m_Result[nResultNum - 1];
                }
                else
                {
                    pTeammember = *(it - 1);
...
int KPlayer::LuaStopCurrentAction(Lua_State* L)
{
    ...
    if (
        (m_OTActionParam.eActionType != otActionIdle && m_OTActionParam.eActionType != otActionPicking) || // otActionPicking��ͨ��Esc�����
...
int KPlayer::LuaSetPartyLootMode(Lua_State* L)
{
	...
	KGLOG_PROCESS_ERROR(m_dwTeamID != ERROR_ID); // System Team�������ã�����ֻ��������ͨ����
...
int KPlayer::LuaTradingInviteRequest(Lua_State* L)
{
    ...
    nRetCode = g_InRange(this, pInviteDst, COMMON_PLAYER_OPERATION_DISTANCE);
...
int KPlayer::LuaTalk(Lua_State* L)
{
    ...
    if (nTopIndex == 2)
        nChannel = (int)Lua_ValueToNumber(L, 2);

    KGLOG_PROCESS_ERROR(nChannel == trNearby || nChannel == trSence);

    uBuffLen = PainText2TalkData(MAX_CHAT_TEXT_LEN, byBuff, pszText);
    KGLOG_PROCESS_ERROR(uBuffLen);

    bRetCode = g_PlayerServer.DoTalkMessage(nChannel, m_dwID, m_szName, ERROR_ID, NULL, uBuffLen, byBuff);
    KGLOG_PROCESS_ERROR(bRetCode);
...
static inline int _FillTalkData(Lua_State* L, int nTableIndex, BYTE* pbyBuf, unsigned uBufSize)
{
    ...
    while (Lua_Next(L, nTableIndex))
    {
		...
		if (!strcmp(pszType, "achievement"))
		{
			...
			KGLOG_PROCESS_SUCCESS((pbyBufEnd + sizeof(BYTE)) + sizeof(dwID) > pbyBuf + uBufSize);

			*pbyBufEnd = (BYTE)tdbLinkAchievement;
			pbyBufEnd += sizeof(BYTE);
			memcpy(pbyBufEnd, &dwID, sizeof(dwID));
			pbyBufEnd += sizeof(dwID);
		}

		if (!strcmp(pszType, "designation"))
		{
			DWORD dwID = 0;
			DWORD dwPrefix = 0;
			...
			dwPrefix = (DWORD)(BOOL)Lua_ValueToBoolean(L, -1);
			lua_pop(L, 1);

			KGLOG_PROCESS_SUCCESS((pbyBufEnd + sizeof(BYTE)) + sizeof(dwID) + sizeof(dwPrefix) > pbyBuf + uBufSize);

			*pbyBufEnd = (BYTE)tdbLinkDesignation;
			pbyBufEnd += sizeof(BYTE);
			memcpy(pbyBufEnd, &dwID, sizeof(dwID));
			pbyBufEnd += sizeof(dwID);
			memcpy(pbyBufEnd, &dwPrefix, sizeof(dwPrefix));
			pbyBufEnd += sizeof(dwPrefix);
		}

        if (!strcmp(pszType, "eventlink"))
        {
            char *pszLink = NULL;
            char *pszName = NULL;
            size_t uLinkLen = 0;
            ...
            lua_getfield(L, -1, "linkinfo");
            pszLink = (char *)Lua_ValueToString(L, -1);
            KGLOG_PROCESS_ERROR(pszLink);
            lua_pop(L, 1);
            
            uNameLen = strlen(pszName);
            uLinkLen = strlen(pszLink);
            KGLOG_PROCESS_SUCCESS((pbyBufEnd + sizeof(BYTE) + uNameLen + sizeof('\0') + uLinkLen + sizeof('\0')) > pbyBuf + uBufSize);

            *pbyBufEnd = (BYTE)tdbEventLink;
            pbyBufEnd += sizeof(BYTE);
            memcpy(pbyBufEnd, pszName, uNameLen);
            pbyBufEnd += uNameLen;
            *pbyBufEnd = '\0';
            pbyBufEnd += sizeof('\0');

            memcpy(pbyBufEnd, pszLink, uLinkLen);
            pbyBufEnd += uLinkLen;
            *pbyBufEnd = '\0';
            pbyBufEnd += sizeof('\0');
        }
        ...
        case tdbLinkItem:
            {
                ...
                pDestItem = 
                    g_pSO3World->m_ItemManager.GenerateItemFromBinaryData(
                        ERROR_ID, pTalkSyncItem->byData, pTalkSyncItem->uDataSize
                    );
        ...
        case tdbEventLink:
            {
                ...
                pszLink = (char *)(pbyTalkData + uTotalLen);
                KGLOG_PROCESS_ERROR(pszLink);
                uLinkLen = strlen(pszLink);
...
int KPlayer::LuaTalk(Lua_State* L)
{
    ...
    if (nCoolDownID && !m_TimerList.CheckTimer(nCoolDownID))  // ����������CD
    {
        if (g_pGameWorldUIHandler)
        {
            KUIEventResponseMsg UIEventResponseMsg;
            UIEventResponseMsg.nRespondCode = TALK_ERROR_E_CD;
    ...
    if (nTalkRange == trForce && !m_dwForceID)
    {
        if (g_pGameWorldUIHandler)
        {
            KUIEventResponseMsg UIEventResponseMsg;
            UIEventResponseMsg.nRespondCode = TALK_ERROR_E_NOT_IN_FORCE;
            g_pGameWorldUIHandler->OnResponseMsg(UIEventResponseMsg);
        }
...
#ifdef _SERVER
    g_LogClient.LogPlayerDelFriend(this, dwAlliedPlayerID, pFellowship->m_szName);
#endif // _SERVER
...
#ifdef _SERVER
    g_LogClient.LogPlayerDelFoe(this, dwAlliedPlayerID, pFoe->m_szName);
#endif // _SERVER
...
int KPlayer::LuaAddFellowship(Lua_State* L)
{
    ...
    pFellowship = g_pSO3World->m_FellowshipMgr.AddFellowship(m_dwID, dwAlliedPlayerID, pszAlliedPlayerName, true);
    KG_PROCESS_ERROR(pFellowship);

    g_pSO3World->m_FellowshipMgr.ProcessFellowshipCountAchievement(this);

    g_LogClient.LogPlayerAddFriend(this, dwAlliedPlayerID, pszAlliedPlayerName);
...
int KPlayer::LuaAddFellowshipAttraction(Lua_State* L)
{
    ...
    if (nTopIndex == 3)
        bBidirectional = Lua_ValueToBoolean(L, 3);

    bRetCode = g_pSO3World->m_FellowshipMgr.AddFellowshipAttraction(m_dwID, dwAlliedPlayerID, nAttraction, bBidirectional);
    KGLOG_PROCESS_ERROR(bRetCode);
...
int KPlayer::LuaAddFoe(Lua_State* L)
{
    ...
    g_LogClient.LogPlayerAddFoe(this, dwAlliedPlayerID, pszAlliedPlayerName);
...
int KPlayer::LuaGetFellowshipGroupInfo(Lua_State* L)
{
    ...
    {
        strncpy(szGroupName, it->second.szGroupName[i], sizeof(szGroupName));// ���ѷ�������
        szGroupName[sizeof(szGroupName) - sizeof('\0')] = '\0';
...
int KPlayer::LuaSendSystemMessage(Lua_State* L)
{
    ...
    (void)pbyBuffer; /*[endgame] tolerant*/
...
int KPlayer::LuaAddEnchant(Lua_State* L)
{
    ...
    ENCHANT_RESULT_CODE   nRetCode    = ercFailed;
    DWORD                 dwBoxIndex  = 0;
    DWORD                 dwPos       = 0;
    int                   nEnchantID  = 0;
    int                   nTopIndex   = Lua_GetTopIndex(L);
    ...
    nRetCode = m_ItemList.AddEnchant(dwBoxIndex, dwPos, nEnchantID);
    KG_PROCESS_ERROR_RET_CODE(nRetCode == ercSuccess, nRetCode);
...
int KPlayer::LuaRemoveEnchant(Lua_State* L)
{
	...
	dwBoxIndex          = (DWORD)Lua_ValueToNumber(L, 1);
	dwPos               = (DWORD)Lua_ValueToNumber(L, 2);
    nEnchantIndex       = (int)Lua_ValueToNumber(L, 3);

    nRetCode = m_ItemList.RemoveEnchant(dwBoxIndex, dwPos, nEnchantIndex);
    KG_PROCESS_ERROR_RET_CODE(nRetCode == ercSuccess, nRetCode);
...
int KPlayer::LuaMountItem(Lua_State* L)
{
    BOOL              bResult     = false;
    int               nRetCode    = false;
    DWORD             dwSrcBox    = 0;
    ...
    dwSrcBox    = (DWORD)Lua_ValueToNumber(L, 1);
    dwSrcPos    = (DWORD)Lua_ValueToNumber(L, 2);
    dwDestBox   = (DWORD)Lua_ValueToNumber(L, 3);
    dwDestPos   = (DWORD)Lua_ValueToNumber(L, 4);

    nRetCode = m_ItemList.CanMountItem(dwSrcBox, dwSrcPos, dwDestBox, dwDestPos);
    KGLOG_PROCESS_ERROR(nRetCode == ercSuccess);

    pSrcItem = m_ItemList.GetItem(dwSrcBox, dwSrcPos);
    KGLOG_PROCESS_ERROR(pSrcItem);
    KGLOG_PROCESS_ERROR(pSrcItem->m_Common.nGenre == igMountItem);
    KGLOG_PROCESS_ERROR(pSrcItem->m_GenParam.dwTabType == ittOther);
    
    g_LogClient.GetItemLogInfo(pSrcItem, &ItemLogInfo);
    pItemInfo = (KOtherItemInfo*)g_pSO3World->m_ItemManager.GetItemInfo(pSrcItem->m_GenParam.dwTabType, pSrcItem->m_GenParam.dwIndex);
    KGLOG_PROCESS_ERROR(pItemInfo);

    nRetCode = m_ItemList.AddEnchant(dwDestBox, dwDestPos, pItemInfo->dwEnchantID, pSrcItem);
    KGLOG_PROCESS_ERROR(nRetCode == ercSuccess);
    
    Pos.dwBox = dwSrcBox;
    Pos.dwX   = dwSrcPos;
    
    nRetCode = m_ItemList.DestroyItem(Pos);
    KGLOG_PROCESS_ERROR(nRetCode == ircSuccess);
	
	g_LogClient.LogPlayerItemChange(this, ITEM_OPERATION_TYPE_OUT, &ItemLogInfo, "mount item");
...
int KPlayer::LuaUnMountItem(Lua_State* L)
{
    BOOL              bResult     = false;
    int               nRetCode    = false;
    DWORD             dwSrcBox    = 0;
    ...
    dwSrcBox    = (DWORD)Lua_ValueToNumber(L, 1);
    dwSrcPos    = (DWORD)Lua_ValueToNumber(L, 2);
    nMountIndex = (int)Lua_ValueToNumber(L, 3);

    KGLOG_PROCESS_ERROR(nMountIndex >= eiMount1 && nMountIndex <= eiMount4);

    nRetCode = m_ItemList.RemoveEnchant(dwSrcBox, dwSrcPos, nMountIndex);
    KGLOG_PROCESS_ERROR(nRetCode == ercSuccess);
...
int KPlayer::LuaCanMountItem(Lua_State* L)
{
    int               nResult     = false;
    int               nRetCode    = false;
    DWORD             dwSrcBox    = 0;
...
int KPlayer::LuaCanUnMountItem(Lua_State* L)
{
    int   nResult     = false;
    int   nRetCode    = false;
    DWORD dwSrcBox    = 0;
    ...
    nRetCode = m_ItemList.CanUnMountItem(dwSrcBox, dwSrcPos, nMountIndex);
    KG_PROCESS_ERROR_RET_CODE(nRetCode == ercSuccess, nRetCode);
...
int KPlayer::LuaIsAchievementAcquired(Lua_State* L)
{
    ...
    bResult = m_Achievement.IsAchieve(nAchievementID);
...
int KPlayer::LuaAcquireAchievement(Lua_State* L)
{
    ...
    bRetCode = m_Achievement.Acquire(nAchievementID);
    KG_PROCESS_ERROR(bRetCode);
...
int KPlayer::LuaAddAchievementCount(Lua_State* L)
{
    BOOL    bResult     = false;
    BOOL    bRetCode    = false;
    int     nTopIndex   = 0;
    ...
    nDeltaCount = (int)Lua_ValueToNumber(L, 2);
    KGLOG_PROCESS_ERROR(nDeltaCount >= 0);

    bRetCode = m_Achievement.AddCount(nID, nDeltaCount);
    KGLOG_PROCESS_ERROR(bRetCode);
...
int KPlayer::LuaGetAchievementPoint(Lua_State* L)
{
    ...
    Lua_PushNumber(L, m_Achievement.m_nPoint);
...
int KPlayer::LuaGetAchievementRecord(Lua_State* L)
{
    ...
    Lua_PushNumber(L, m_Achievement.m_nRecord);
...
int KPlayer::LuaGetAchievementCount(Lua_State* L)
{
    int nResult = 0;
    int nTopIndex = 0;
    int nID = 0;
    ...
    nCount = m_Achievement.GetCount(nID);
...
int KPlayer::LuaSetCurrentKillPoint(Lua_State* L)
{
    ...
    nNewKillPoint = (int)Lua_ValueToNumber(L, 1);
    
    if (m_nCurrentKillPoint == 0 && nNewKillPoint > 0)
    {
        m_nNextKillPointReduceTime = g_pSO3World->m_nCurrentTime + g_pSO3World->m_Settings.m_ConstList.nKillPointReduceCycle;
    }

    m_nCurrentKillPoint = nNewKillPoint;
...
int KPlayer::LuaAddPrestige(Lua_State* L)
{
    ...
    g_pSO3World->m_StatDataServer.UpdatePrestigeStat(this, nAddPrestige, "OTHER");
Exit0:
    return 0;
}
...
int KPlayer::LuaCanOpenCampFlag(Lua_State* L)
{
    BOOL bResult = false;
    
    bResult = CanOpenCampFlag();
Exit0:
    Lua_PushBoolean(L, bResult);
    return 1;
}

int KPlayer::LuaCanCloseCampFlag(Lua_State* L)
{
    BOOL bResult = false;

    bResult = CanCloseCampFlag();
Exit0:
    Lua_PushBoolean(L, bResult);
    return 1;
}
...
#ifdef _SERVER
int KPlayer::LuaOpenCampFlag(Lua_State* L)
{
    BOOL bRetCode = false;

    bRetCode = CanOpenCampFlag();
    KG_PROCESS_ERROR(bRetCode);
...
int KPlayer::LuaCloseCampFlag(Lua_State* L)
{
    BOOL bResult  = false;
    BOOL bRetCode = false;

    bRetCode = CanCloseCampFlag();
    KG_PROCESS_ERROR(bRetCode);

    m_nCloseCampFlagTime = g_pSO3World->m_nCurrentTime + CLOSE_CAMP_FLAG_TIME;
    
    bResult = true;
Exit1:
    g_pSO3World->m_ScriptServer.Reset();
    g_pSO3World->m_ScriptServer.PushBool(bResult);
    g_pSO3World->m_ScriptServer.PushNumber(CLOSE_CAMP_FLAG_TIME);
    g_pSO3World->m_ScriptServer.Call(m_nConnIndex, "CloseCampFlagResult");
...
int KPlayer::LuaGetReviveCtrl(Lua_State* L)
{
    ...
    Lua_PushString(L, "nMessageID");
    Lua_PushNumber(L, m_ReviveCtrl.nMessageID);
    Lua_SetTable(L, -3);
...
int KPlayer::LuaSetReviveCtrl(Lua_State* L)
{
    ...
    lua_getfield(L, -1, "nMessageID");
    KGLOG_PROCESS_ERROR(Lua_IsNumber(L, -1));
    NewReviveCtrl.nMessageID = (int)Lua_ValueToNumber(L, -1);
    Lua_Pop(L, 1);

    nRetCode = memcmp(&m_ReviveCtrl, &NewReviveCtrl, sizeof(m_ReviveCtrl));
...
int KPlayer::LuaCreateViewPoint(Lua_State* L)
{
    int         nTopIndex   = Lua_GetTopIndex(L);
    int         nX          = 0;
    int         nY          = 0;
    ...
    nY      	= (int)Lua_ValueToNumber(L, 2);
    nZ      	= (int)Lua_ValueToNumber(L, 3);
    nLifeFrame  = (int)Lua_ValueToNumber(L, 4);

    KGLOG_PROCESS_ERROR(nX >= 0);
    KGLOG_PROCESS_ERROR(nY >= 0);
    KGLOG_PROCESS_ERROR(nLifeFrame > 0);
    
    KGLOG_PROCESS_ERROR(m_ViewPoint.pRegion == NULL);

    pRegion = m_pScene->GetRegionByPoint(nX, nY);
    KGLOG_PROCESS_ERROR(pRegion);

    m_ViewPoint.pRegion = pRegion;
    m_ViewPoint.nDisappearFrame = g_pSO3World->m_nGameLoop + nLifeFrame;

    pRegion->m_ViewPointList.AddTail((KNode*)&m_ViewPoint);

    g_PlayerServer.DoCreateViewPointRespond(this);

    m_pScene->ValidateRegions(pRegion->m_nRegionX, pRegion->m_nRegionY);
    RegisterViewObjectByViewPoint(NULL, pRegion);

Exit0:
    return 0;
}

int KPlayer::LuaRemoveViewPoint(Lua_State* L)
{
    int		nTopIndex = Lua_GetTopIndex(L);

    KGLOG_PROCESS_ERROR(nTopIndex == 0);
    KGLOG_PROCESS_ERROR(m_ViewPoint.pRegion);
...
int KPlayer::LuaIsViewPointExist(Lua_State* L)
{
    BOOL    bResult   = false;
    int     nTopIndex = Lua_GetTopIndex(L);
...
#ifdef _CLIENT
int KPlayer::LuaHideHat(Lua_State* L)
{   
    int     nTopIndex    = 0;
...
int KPlayer::LuaSetCurrentDesignation(Lua_State* L)
{
    BOOL bRetCode       = 0;
    int  nPrefix        = 0;
    int  nPostfix       = 0;
    ...
    nPostfix        = (int)Lua_ValueToNumber(L, 2);
    bDisplayFlag    = (BOOL)Lua_ValueToBoolean(L, 3);

#ifdef _CLIENT
    g_PlayerClient.DoSetCurrentDesignationRequest(nPrefix, nPostfix, bDisplayFlag);
#endif
...
int KPlayer::LuaGetAcquiredDesignationPrefix(Lua_State* L)
{
    BOOL bRetCode   = false;
    int  nIndex     = 1;
    ...
    for (int nPrefix = 1; nPrefix <= MAX_DESIGNATION_FIX_ID; nPrefix++)
    {
        bRetCode = m_Designation.IsPrefixAcquired(nPrefix);
        if (!bRetCode)
            continue;

        Lua_PushNumber(L, nIndex);
        Lua_PushNumber(L, nPrefix);
        Lua_SetTable(L, -3);
...
int KPlayer::LuaGetAcquiredDesignationPostfix(Lua_State* L)
{
    BOOL bRetCode   = false;
    int  nIndex     = 1;
    ...
    for (int nPostfix = 1; nPostfix <= MAX_DESIGNATION_FIX_ID; nPostfix++)
    {
        bRetCode = m_Designation.IsPostfixAcquired(nPostfix);
        if (!bRetCode)
            continue;

        Lua_PushNumber(L, nIndex);
        Lua_PushNumber(L, nPostfix);
        Lua_SetTable(L, -3);
...
int KPlayer::LuaIsDesignationPrefixAcquired(Lua_State* L)
{
    BOOL bResult    = false;
    BOOL bRetCode   = false;
    int  nPrefix    = 0;
...
int KPlayer::LuaIsDesignationPostfixAcquired(Lua_State* L)
{
    BOOL bResult    = false;
    BOOL bRetCode   = false;
    int  nPostfix  = 0;
...
#ifdef _SERVER
int KPlayer::LuaAcquireDesignationPrefix(Lua_State* L)
{
    BOOL bRetCode   = false;
    int  nPrefix    = 0;
    int  nTopIndex  = Lua_GetTopIndex(L);
    ...
    bRetCode = m_Designation.AcquirePrefix(nPrefix);
    KGLOG_PROCESS_ERROR(bRetCode);
...
int KPlayer::LuaAcquireDesignationPostfix(Lua_State* L)
{
    BOOL bResult    = false;
    BOOL bRetCode   = false;
    int  nPostfix  = 0;
    ...
    bRetCode = m_Designation.AcquirePostfix(nPostfix);
    KGLOG_PROCESS_ERROR(bRetCode);
...
int KPlayer::LuaRemoveDesignationPrefix(Lua_State* L)
{
    int  bResult    = false;
    BOOL bRetCode   = 0;
    int  nPrefix    = 0;
    ...
    bRetCode = m_Designation.RemovePrefix(nPrefix);
    KGLOG_PROCESS_ERROR(bRetCode);
...
int KPlayer::LuaRemoveDesignationPostfix(Lua_State* L)
{
    int  bResult    = false;
    BOOL bRetCode   = 0;
    int  nPostfix   = 0;
    ...
    bRetCode = m_Designation.RemovePostfix(nPostfix);
    KGLOG_PROCESS_ERROR(bRetCode);
...
int KPlayer::LuaSetGeneration(Lua_State* L)
{
    BOOL bRetCode           = 0;
    ...
    KGLOG_PROCESS_ERROR(nTopIndex == 1);
    nGenerationIndex = (int)Lua_ValueToNumber(L, 1);

    bRetCode = m_Designation.SetGeneration(nGenerationIndex);
    KGLOG_PROCESS_ERROR(bRetCode);   
...
int KPlayer::LuaGetCurrentDesignationPrefix(Lua_State* L)
{
    Lua_PushNumber(L, m_Designation.m_nCurrentPrefix);

    return 1;
}

int KPlayer::LuaGetCurrentDesignationPostfix(Lua_State* L)
{
    Lua_PushNumber(L, m_Designation.m_nCurrentPostfix);

    return 1;
}

int KPlayer::LuaGetDesignationGeneration(Lua_State* L)
{
    Lua_PushNumber(L, m_Designation.m_nGenerationIndex);

    return 1;
}

int KPlayer::LuaGetDesignationByname(Lua_State* L)
{
    Lua_PushNumber(L, m_Designation.m_nBynameIndex);

    return 1;
}

int KPlayer::LuaGetDesignationDisplayFlag(Lua_State* L)
{
    Lua_PushBoolean(L, m_Designation.m_bDisplayFlag);
...
int KPlayer::LuaGetAcquiredDesignationCount(Lua_State* L)
{
	...
	for (int nPrefix = 1; nPrefix <= MAX_DESIGNATION_FIX_ID; nPrefix++)
	{
		if (m_Designation.IsPrefixAcquired(nPrefix))
		{
			nCount++;
		}
	}
	...
	for (int nPostfix = 1; nPostfix <= MAX_DESIGNATION_FIX_ID; nPostfix++)
	{
		if (m_Designation.IsPostfixAcquired(nPostfix))
		{
			nCount++;
		}
	}
...
#ifdef _CLIENT
int KPlayer::LuaGetCurrentDesignation(Lua_State* L)
{
    int             nResult                 = 0;
    int		        nTopIndex               = Lua_GetTopIndex(L);
...
int KPlayer::LuaSetDesignationContent(Lua_State* L)
{
	int             nResult                 = 0;
	BOOL			bUpdateRepresent		= TRUE;
	int		        nTopIndex               = Lua_GetTopIndex(L);
	const char *	pcszDesignation			= NULL;

	KGLOG_PROCESS_ERROR(nTopIndex > 0);

	pcszDesignation = Lua_ValueToString(L, 1);
	if(!pcszDesignation)
		pcszDesignation = "";

	if (nTopIndex > 1)
		bUpdateRepresent = (BOOL)Lua_ValueToBoolean(L, 2);

	strncpy(m_szTitle, pcszDesignation, _countof(m_szTitle));
	m_szTitle[_countof(m_szTitle) - 1] = '\0';

	if (bUpdateRepresent &&g_pGameWorldRepresentHandler)
...
#ifdef _SERVER
int KPlayer::LuaIsVenationSuccess(Lua_State* L)
{
    int         nResult      = 0;
    BOOL        bRetCode     = false;
    int         nTopIndex    = 0;
    ...
    dwSkillID    = (DWORD)Lua_ValueToNumber(L, 1);
    dwSkillLevel = (DWORD)Lua_ValueToNumber(L, 2);
    uProbability = (unsigned)Lua_ValueToNumber(L, 3);

    KGLOG_PROCESS_ERROR(dwSkillID > 0);
    KGLOG_PROCESS_ERROR(dwSkillLevel > 0);
    KGLOG_PROCESS_ERROR(uProbability > 0);

    bRetCode = IsVenationSuccess(dwSkillID, dwSkillLevel, uProbability);
    Lua_PushBoolean(L, bRetCode);
...
int KPlayer::LuaActivePresentCode(Lua_State* L)
{
    BOOL        bRetCode        = false;
    int         nTopIndex       = 0;
    const char* pszPresentCode  = NULL;

    nTopIndex = Lua_GetTopIndex(L);
    KGLOG_PROCESS_ERROR(nTopIndex == 1);

    pszPresentCode = Lua_ValueToString(L, 1);
    KGLOG_PROCESS_ERROR(pszPresentCode);

    KGLogPrintf(
        KGLOG_INFO, "Apply Active PresentCode. PlayerID = %d, PresentCode = %s",
        m_dwID, pszPresentCode
    );

    bRetCode = g_RelayClient.DoActivePresentCodeRequest(this, pszPresentCode);
    KGLOG_PROCESS_ERROR(bRetCode);
...
int KPlayer::LuaGetChargeFlag(Lua_State* L)
{
    int nResult     = 0;
    int nTopIndex   = 0;
    ...
    Lua_PushBoolean(L, m_bChargeFlag);
    nResult = 1;
Exit0:
    return nResult;
}

int KPlayer::LuaIsGM(Lua_State* L)
{
    BOOL bIsGM      = false;

    bIsGM = g_pSO3World->m_Settings.m_GMList.CheckPermission(m_szAccount);

    Lua_PushBoolean(L, bIsGM);

    return 1;
}

int KPlayer::LuaPreemptiveAttack(Lua_State* L)
{
    int                     nResult         = paInvalid;
    int                     nTopIndex       = Lua_GetTopIndex(L);
    DWORD                   dwTargetID      = ERROR_ID;
    KCharacter*             pTarget         = NULL;
    const KSimpThreatNode*  cpThreatNode    = NULL;
    const KSimpThreatNode*  cpSelfNode      = NULL;

    KGLOG_PROCESS_ERROR(nTopIndex == 1);

    dwTargetID = (DWORD)Lua_ValueToNumber(L, 1);
    KGLOG_PROCESS_ERROR(dwTargetID != m_dwID);

    if (IS_PLAYER(dwTargetID))
        pTarget = (KCharacter*)g_pSO3World->m_PlayerSet.GetObj(dwTargetID);
    else
        pTarget = (KCharacter*)g_pSO3World->m_NpcSet.GetObj(dwTargetID);

    KGLOG_PROCESS_ERROR(pTarget);
    KG_PROCESS_ERROR(pTarget->m_pScene);
    ...
    else
    {
        ...
        nResult = cpSelfNode->bPrimacord ? paSelf : paTarget;
    }
...
int KPlayer::LuaAddMaxApprenticeNum(Lua_State* L)
{
    int nTopIndex   = 0;
    int nDeltaValue = 0;
    ...
    KGLOG_PROCESS_ERROR(nDeltaValue > 0);

    if (m_nMaxApprenticeNum < MAX_APPRENTICE_NUM - nDeltaValue)
    {
        m_nMaxApprenticeNum += nDeltaValue;
    }
    else
    {
        m_nMaxApprenticeNum = MAX_APPRENTICE_NUM;
    }
...
#ifdef _SERVER
    bRetCode = g_pSO3World->m_MentorCache.IsMentorship(dwMentorID, m_dwID);
#else
    bRetCode = g_pSO3World->m_MentorClient.IsMyMentor(dwMentorID);
#endif
    KG_PROCESS_ERROR(bRetCode);
...
int KPlayer::LuaIsMyApprentice(Lua_State* L)
{
    BOOL    bResult         = false;
    BOOL    bRetCode        = false;
    int     nTopIndex       = 0;
...
#ifdef _SERVER
    bRetCode = g_pSO3World->m_MentorCache.IsMentorship(m_dwID, dwApprenticeID);
#else
    bRetCode = g_pSO3World->m_MentorClient.IsMyApprentice(dwApprenticeID);
#endif
    KG_PROCESS_ERROR(bRetCode);
...
int KPlayer::LuaIsMentor(Lua_State* L)
{
    BOOL    bResult         = false;
    BOOL    bRetCode        = false;

#ifdef _CLIENT 
    bRetCode = g_pSO3World->m_MentorClient.IsMentor();
#else
    bRetCode = g_pSO3World->m_MentorCache.IsMentor(m_dwID);
#endif
    KG_PROCESS_ERROR(bRetCode);
...
int KPlayer::LuaIsApprentice(Lua_State* L)
{
    BOOL    bResult         = false;
    BOOL    bRetCode        = false;
...
#else
    bRetCode = g_pSO3World->m_MentorCache.IsApprentice(m_dwID);
#endif
    KG_PROCESS_ERROR(bRetCode);
...
int KPlayer::LuaCanLootBoxItem(Lua_State* L)
{
    BOOL    bResult         = false;
    BOOL    bRetCode        = false;
    int     nTopIndex       = 0;
    ...
    pBox = m_ItemList.GetItem(dwBox, dwX);
    KGLOG_PROCESS_ERROR(pBox);

    bRetCode = m_ItemList.CanLootBoxItem(pBox->m_dwID);
    KG_PROCESS_ERROR(bRetCode == aircSuccess);
...
int KPlayer::LuaOpenBox(Lua_State* L)
{
    BOOL    bResult         = false;
    BOOL    bRetCode        = false;
    int     nTopIndex       = 0;
    ...
    bRetCode = m_ItemList.OpenBox(pBox);
    KGLOG_PROCESS_ERROR(bRetCode);
...
#ifdef _SERVER
int KPlayer::LuaLootBoxItem(Lua_State* L)
{
    BOOL    bResult         = false;
    BOOL    bRetCode        = false;
    int     nTopIndex       = 0;
    ...
    bRetCode = m_ItemList.LootBoxItem(dwBox, dwX);
    KG_PROCESS_ERROR(bRetCode);
...
int KPlayer::LuaSetItemGenTime(Lua_State* L)
{
    BOOL    bResult         = false;
    BOOL    bRetCode        = false;
    int     nTopIndex       = 0;
    ...
    dwBox = (DWORD)Lua_ValueToNumber(L, 1);
    dwX   = (DWORD)Lua_ValueToNumber(L, 2);
    nGenTime = (time_t)Lua_ValueToNumber(L, 3);
    ...
    g_PlayerServer.DoSyncItemData(
        m_nConnIndex, m_dwID, pItem, dwBox, dwX
    );
...
#ifdef _CLIENT
int KPlayer::LuaGetBoxItem(Lua_State* L)
{
    int    nRetCode        = false;
    int    nIndex          = 1;
    Lua_NewTable(L);

    for (size_t i = 0; i < m_ItemList.m_BoxItem.size(); ++i)
    {
        if (m_ItemList.m_BoxItem[i] == NULL)
            continue;
        
        Lua_PushNumber(L, nIndex++);
        m_ItemList.m_BoxItem[i]->LuaGetObj(L);
...
DEFINE_LUA_CLASS_BEGIN(KPlayer)
    ...
    REGISTER_LUA_INTEGER_READONLY(KCharacter, BattleFieldSide)
    ...
    REGISTER_LUA_BOOL_READONLY(KCharacter, PositiveShield)
    REGISTER_LUA_BOOL_READONLY(KCharacter, NegativeShield)
    ...
    REGISTER_LUA_INTEGER(KCharacter, PhysicsCriticalDamagePowerBaseKiloNumRate)
    REGISTER_LUA_INTEGER(KCharacter, PhysicsCriticalDamagePower)
    ...
    REGISTER_LUA_INTEGER(KCharacter, SolarCriticalDamagePowerBaseKiloNumRate)
    REGISTER_LUA_INTEGER(KCharacter, SolarCriticalDamagePower)
    ...
    REGISTER_LUA_INTEGER(KCharacter, NeutralCriticalDamagePowerBaseKiloNumRate)
    REGISTER_LUA_INTEGER(KCharacter, NeutralCriticalDamagePower)
    ...
    REGISTER_LUA_INTEGER(KCharacter, LunarCriticalDamagePowerBaseKiloNumRate)
    REGISTER_LUA_INTEGER(KCharacter, LunarCriticalDamagePower)
    ...
    REGISTER_LUA_INTEGER(KCharacter, PoisonCriticalDamagePowerBaseKiloNumRate)
    REGISTER_LUA_INTEGER(KCharacter, PoisonCriticalDamagePower)
...
#ifdef _SERVER
    REGISTER_LUA_STRING_READONLY(KPlayer, ClientIP)
    ...
    REGISTER_LUA_INTEGER_READONLY(KPlayer, Coin)

#ifdef _CLIENT
    REGISTER_LUA_DWORD(KPlayer, FormationEffectID)
    REGISTER_LUA_INTEGER(KPlayer, FormationEffectLevel)

    REGISTER_LUA_DWORD(KPlayer, MentorFormationEffectID)
    REGISTER_LUA_INTEGER(KPlayer, MentorFormationEffectLevel)
...
#ifdef  _SERVER
    REGISTER_LUA_BOOL(KPlayer, FarmerLimit)
#endif
...
#ifdef _SERVER
    REGISTER_LUA_FUNC(KPlayer, HaveItem)
    REGISTER_LUA_FUNC(KPlayer, CostSingleItemByID)
    REGISTER_LUA_FUNC(KPlayer, CanAddItem)
    REGISTER_LUA_FUNC(KPlayer, AddItem)
    REGISTER_LUA_FUNC(KPlayer, CostItem)
    REGISTER_LUA_FUNC(KPlayer, CostItemInAllPackage)
    ...
    REGISTER_LUA_FUNC(KPlayer, GetTotalGameTime)
    REGISTER_LUA_FUNC(KPlayer, GetCreateTime)
#endif
    ...
    REGISTER_LUA_FUNC(KPlayer, GetSkillTarget)
    ...
    REGISTER_LUA_FUNC(KPlayer, GetQuestReputationInfo)
#endif
    ...
    REGISTER_LUA_FUNC(KPlayer, GetProfessionAdjustLevel)
...
#ifdef _SERVER
    REGISTER_LUA_FUNC(KPlayer, ForgetRecipe)
#endif
    ...
    REGISTER_LUA_FUNC(KPlayer, ClearCDTime)
    ...
    REGISTER_LUA_FUNC(KPlayer, MountItem)
    REGISTER_LUA_FUNC(KPlayer, UnMountItem)
#endif
    REGISTER_LUA_FUNC(KPlayer, CanMountItem)
    REGISTER_LUA_FUNC(KPlayer, CanUnMountItem)
...
#ifdef _SERVER
    REGISTER_LUA_FUNC(KCharacter, Stealth)
#endif
    ...
    REGISTER_LUA_FUNC(KPlayer, CanOpenCampFlag)
    REGISTER_LUA_FUNC(KPlayer, CanCloseCampFlag)

#ifdef _SERVER
    REGISTER_LUA_FUNC(KPlayer, OpenCampFlag)
    REGISTER_LUA_FUNC(KPlayer, CloseCampFlag)
    ...
    REGISTER_LUA_FUNC(KPlayer, AddAchievementCount)
	
	REGISTER_LUA_FUNC(KPlayer, CreateViewPoint)
    REGISTER_LUA_FUNC(KPlayer, RemoveViewPoint)
    REGISTER_LUA_FUNC(KPlayer, IsViewPointExist)
#endif
    REGISTER_LUA_FUNC(KPlayer, GetAchievementPoint)
    REGISTER_LUA_FUNC(KPlayer, GetAchievementRecord)
    REGISTER_LUA_FUNC(KPlayer, GetAchievementCount)

    REGISTER_LUA_FUNC(KPlayer, SetCurrentDesignation)
    REGISTER_LUA_FUNC(KPlayer, GetAcquiredDesignationPrefix)
    REGISTER_LUA_FUNC(KPlayer, GetAcquiredDesignationPostfix)
    REGISTER_LUA_FUNC(KPlayer, IsDesignationPrefixAcquired)
    REGISTER_LUA_FUNC(KPlayer, IsDesignationPostfixAcquired)
#ifdef _SERVER
    REGISTER_LUA_FUNC(KPlayer, AcquireDesignationPrefix)
    REGISTER_LUA_FUNC(KPlayer, AcquireDesignationPostfix)
    ...
    REGISTER_LUA_FUNC(KPlayer, GetCurrentDesignationPrefix)
    REGISTER_LUA_FUNC(KPlayer, GetCurrentDesignationPostfix)
    ...
    REGISTER_LUA_FUNC(KPlayer, SetDesignationContent)
#endif
...
// ʦͽ
    REGISTER_LUA_FUNC(KPlayer, IsMyMentor)
    REGISTER_LUA_FUNC(KPlayer, IsMyApprentice)
    REGISTER_LUA_FUNC(KPlayer, IsMentor)
    REGISTER_LUA_FUNC(KPlayer, IsApprentice)

    REGISTER_LUA_FUNC(KPlayer, CanLootBoxItem)
    REGISTER_LUA_FUNC(KPlayer, OpenBox)
#ifdef _SERVER
    REGISTER_LUA_FUNC(KPlayer, LootBoxItem)
    REGISTER_LUA_FUNC(KPlayer, SetItemGenTime)
#endif
#ifdef _CLIENT
    REGISTER_LUA_FUNC(KPlayer, GetBoxItem)
#endif
