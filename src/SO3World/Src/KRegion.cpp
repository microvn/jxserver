...
void KRegion::Activate(void)
{
	...
	while (pSceneObjNode != NULL)
	{
        ...
        if (pPlayer->m_bFightState && pPlayer->m_eMoveState != cmsOnDeath)
            nInFightPlayerCount++;
...
void KRegion::MergeCellList(KCell* pHead)
{
    ...
    while (pProcessCell)
    {
        ...
        while (pCellNode)
        {
            ...
            {
                if (pCellNode->m_BaseInfo.dwFaceUp && !pProcessCell->m_BaseInfo.dwFaceUp)
                {
                    pProcessCell->m_BaseInfo = pCellNode->m_BaseInfo;
                }
                else if (pCellNode->m_BaseInfo.dwGradientDegree < pProcessCell->m_BaseInfo.dwGradientDegree)
                {
                    pProcessCell->m_BaseInfo = pCellNode->m_BaseInfo;
                }
            }
    ...
    )
    {
        if (pCellNode->m_BaseInfo.dwCellType == ctWater)
        {
            ...
            if (pCellNode->m_wLowLayer * POINT_PER_ALTITUDE < nFootPosZ)
            {
                ...
                if (pNextRealCell && pNextRealCell->m_wLowLayer * POINT_PER_ALTITUDE < nFootPosZ + nHeight)
                    nRelativeZ -= (nFootPosZ + nHeight) - (pNextRealCell->m_wLowLayer * POINT_PER_ALTITUDE);
            }
            else
...
#ifdef _SERVER
	if (pNpc->m_bActiveMark || pNpc->m_bFightState || pNpc->m_AIData.nPatrolPathID > 0)
	{
		SetActive();
	}
#endif
...
BOOL KRegion::DeleteAllObject(void)
{
    ...
    {
        KVIEW_POINT* pNextViewPoint = (KVIEW_POINT*)pViewPoint->GetNext();

		pViewPoint->Remove();
        pViewPoint->pRegion         = NULL;
        pViewPoint->nDisappearFrame = 0;

        pViewPoint = pNextViewPoint;
    }
#endif
...
BOOL KRegisterNewNpcFunc::operator () (KPlayer* pPlayer)
{
    ...
    // 如果之前该player已经通过ViewPoint看到了
    if (m_pFromRegion && pPlayer->m_ViewPoint.pRegion && pPlayer->IsInViewRangeByViewPoint(m_pFromRegion))
        return true;

    InsRet = pPlayer->m_NpcSyncTable.insert(m_pNpc->m_dwID);
    if (InsRet.second)
    {
